...
local function run_query(name, cmd)
  ...
  -- Status: running
  vim.api.nvim_echo(
    { { '[sql-runner] Running ' .. name .. ' query…', 'ModeMsg' } },
    false,
    {}
  )
  ...
  vim.api.nvim_echo({
    {
      string.format('[sql-runner] %s query done in %d ms', name, ms),
      'ModeMsg',
    },
  }, false, {})
end
...
function M.add_command()
  ...
  vim.ui.input({
    ...
    vim.ui.input({
      ...
      vim.notify(
        string.format('[sql-runner] Added command "%s"', alias),
        vim.log.levels.INFO
      )
    end)
  end)
  ...
  local function remove_multiple()
    vim.ui.select(items, {
      ...
      if choice then
        ...
        vim.notify(
          string.format('[sql-runner] Removed command "%s"', choice),
          vim.log.levels.INFO
        )
...
function M.select_command()
  ...
  if vim.tbl_isempty(commands) then
    vim.notify(
      '[sql-runner] No commands configured. Use :AddSqlCmd to add one.',
      vim.log.levels.WARN
    )
    return
  end
  ...
  vim.ui.select(items, {
    ...
    if choice then
      ...
      vim.notify(
        string.format('[sql-runner] Selected "%s"', choice.alias),
        vim.log.levels.INFO
      )
    end
  end)
