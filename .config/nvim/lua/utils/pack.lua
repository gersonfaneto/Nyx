...
---@class minimal.pack.structured_spec.data
...
---@field deps? minimal.pack.spec|minimal.pack.spec[] Dependencies of the plugin, always loaded **before** the main plugin
---@field exts? minimal.pack.spec|minimal.pack.spec[] Extensions of the plugin, always loaded **after** the main plugin
...
---@field build? string|string[]|fun(spec: minimal.pack.spec, path: string)
---@field init? fun(spec: minimal.pack.spec, path: string) Function to call at startup to setup the plugin
...
---@field load? fun(spec: minimal.pack.spec, path: string)
---@field preload? fun(spec: minimal.pack.spec, path: string) Function to execute before loading the plugin
---@field postload? fun(spec: minimal.pack.spec, path: string) Function to execute after loading the plugin
...
---@field keys? minimal.load.key.spec|minimal.load.key.spec[] Keys that lazy-load the plugin
---@field events? minimal.load.event.spec|minimal.load.event.spec[] Events that lazy-load the plugin
---@field cmds? minimal.load.cmd.spec|minimal.load.cmd.spec[] Commands that lazy-load the plugin

---Extended pack spec with lazy-loading
---@class minimal.pack.structured_spec : vim.pack.Spec
---@field data? minimal.pack.structured_spec.data

---@alias minimal.pack.spec string|minimal.pack.structured_spec|vim.pack.Spec

---Merged plugin specs indexed by plugin src
---@type table<string, minimal.pack.structured_spec>
...
function M.load(spec, path)
  ...
  if spec.data.deps then
    ...
    for _, dep in
      ipairs(spec.data.deps --[=[@as minimal.pack.spec[]]=])
    do
  ...
  if spec.data.exts then
    ...
    for _, ext in
      ipairs(spec.data.exts --[=[@as minimal.pack.spec[]]=])
    do
...
---@class (partial) minimal.pack.structured_spec.opts : minimal.pack.structured_spec

---Add specified plugin spec with lazy-loading
---@param specs minimal.pack.spec|minimal.pack.spec[]
---@param default minimal.pack.structured_spec.opts? Default options to merge with the plugin spec table if the plugin is not registered
function M.register(specs, default)
  if not vim.islist(specs) then
    specs = { specs } ---@cast specs minimal.pack.spec[]
  end

  ---@cast specs minimal.pack.structured_spec[]
