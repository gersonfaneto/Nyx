...
require('utils.load').on_events(
  ...
  vim.schedule_wrap(function()
    ...
    map({ 'x', 'n' }, '<Tab>k', '<C-w>gt', { desc = 'Go to next tab' })
    map({ 'x', 'n' }, '<Tab>j', '<C-w>gT', { desc = 'Go to previous tab' })
    map({ 'x', 'n' }, '<Tab>f', '<C-w>gf', { desc = 'Edit file under cursor in new tab' })
    map({ 'x', 'n' }, '<Tab><Tab>', '<C-w>g<Tab>', { desc = 'Go to last accessed tab' })
    map({ 'x', 'n' }, '<Tab>t', '<C-w>T', { desc = 'Move window to new tab' })
    map({ 'x', 'n' }, '<Tab>r', ':TabRename ', { desc = 'Rename the current tab' })
    map({ 'x', 'n' }, '<Tab>e', ':tabedit<CR>', { desc = 'Open new tab' })
    map({ 'x', 'n' }, '<Tab>o', ':tabonly<CR>', { desc = 'Close other tabs' })
    map({ 'x', 'n' }, '<Tab>q', ':tabclose<CR>', { desc = 'Close current tab' })
    map({ 'x', 'n' }, '<Tab>,', ':tabmove -1<CR>', { desc = 'Move tab to the left' })
    map({ 'x', 'n' }, '<Tab>.', ':tabmove +1<CR>', { desc = 'Move tab to the right' })
