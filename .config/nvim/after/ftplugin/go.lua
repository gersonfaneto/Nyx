...
vim.api.nvim_buf_create_user_command(0, 'GoCoverage', function(opts)
  local args = opts.args ~= '' and opts.args or './...'
  local cmd_str = 'go test -coverprofile=coverage.out '
    .. args
...
vim.api.nvim_buf_create_user_command(0, 'GoImpl', function(opts)
  ...
  if impl == '' then
    notify(
      'impl not found. Install with: go install github.com/josharian/impl@latest',
      vim.log.levels.ERROR
    )
    return
  end
...
vim.api.nvim_buf_create_user_command(0, 'GoIfErr', function()
  ...
  local result = vim.fn.system(
    string.format('iferr -pos %d', vim.fn.line2byte(pos[2]) + pos[3] - 1)
  )
  ...
  run_cmd({
    ...
    on_exit = function(result)
      if result.code == 0 then
        ...
        notify(
          'GoModernize failed: ' .. (result.stderr or result.stdout),
          vim.log.levels.ERROR
        )
      end
    end,
...
vim.api.nvim_buf_create_user_command(0, 'GoAddTags', function(opts)
  ...
  local result = vim.fn.system(
    string.format(
      'gomodifytags -file %s -line %d -add-tags %s',
      filepath,
      line,
      tags
    )
...
vim.api.nvim_buf_create_user_command(0, 'GoRemoveTags', function(opts)
  ...
  local result = vim.fn.system(
    string.format(
      'gomodifytags -file %s -line %d -remove-tags %s',
      filepath,
      line,
      tags
    )
