...
What you are now reading is the =README.org= file. It is the
...
- The =minimal-emacs-modules= directory :: It includes all my
  configuration modules. Each module is about a specific type of
  functionality, such a =minimal-emacs-theme.el= for themes and
  =minimal-emacs-essentials.el= for basic tools. These configuration
  modules tweak packages and are not meant to define extra
  functionality.

- The =minimal-lisp= directory :: Here are the custom libraries I
  ...
  meant to be used as part of my setup.

- The =minimal-emacs-pre-custom= file :: It is evaluated /before/
...
- The =minimal-emacs-post-custom= file :: Like the above, this file is
...
- The =README.org= file :: The source of what you are currently
  reading.
...
#+begin_example
~/Developer/Personal/Nyx/.emacs.d $ tree -N -L 4 -C --gitignore --dirsfirst
.
├── minimal-emacs-modules
│   ├── minimal-emacs-completion.el
│   ├── minimal-emacs-dired.el
│   ├── minimal-emacs-doric-themes.el
│   ├── minimal-emacs-ef-themes.el
│   ├── minimal-emacs-essentials.el
│   ├── minimal-emacs-git.el
│   ├── minimal-emacs-icons.el
│   ├── minimal-emacs-langs.el
│   ├── minimal-emacs-mct.el
│   ├── minimal-emacs-modeline.el
│   ├── minimal-emacs-modus-themes.el
│   ├── minimal-emacs-org.el
│   ├── minimal-emacs-search.el
│   ├── minimal-emacs-solarized-themes.el
│   ├── minimal-emacs-standard-themes.el
│   ├── minimal-emacs-theme.el
│   ├── minimal-emacs-vertico.el
│   ├── minimal-emacs-web.el
│   ├── minimal-emacs-which-key.el
│   └── minimal-emacs-window.el
├── minimal-lisp
│   ├── minimal-abbrev.el
│   ├── minimal-comment.el
│   ├── minimal-common.el
...
=README.org= file is not a hindrance in this regard: it provides
...
=README.org= and then type =C-c C-v C-t= (=M-x org-babel-tangle=)
...
** COMMENT The =early-init.el= macro to run code only in a Desktop Environment
:PROPERTIES:
...
(defvar minimal-emacs-tiling-window-manager-regexp "bspwm\\|herbstluftwm\\|i3"
  "Regular expression to  tiling window managers.
See definition of `minimal-emacs-with-desktop-session'.")

(defmacro minimal-emacs-with-desktop-session (&rest body)
  "Expand BODY if desktop session is not a tiling window manager.
See `minimal-emacs-tiling-window-manager-regexp' for what
constitutes a matching tiling window manager."
  (declare (indent 0))
  `(when-let* ((session (getenv "DESKTOP_SESSION"))
               ((not (string-match-p session minimal-emacs-tiling-window-manager-regexp))))
     ,@body))
#+end_src
...
function ~minimal-emacs-no-minibuffer-scroll-bar~.

Another nice thing about the "no toolkit" that I only discovered by
serendipity on 2025-11-23 is that we can use the =(undecorated . t)=
frame parameter /without losing window manager shadows/. This is not
relevant in a tiling window manager, but makes a huge different in a
...
frame name ([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =minimal-emacs-modeline.el= module]]).
...
check the use of ~minimal-pgtk-p~.
...
:PROPERTIES:
:CUSTOM_ID: h:d4e2145a-0aae-4c8e-9518-4cf2ec1a329a
:END:

The ~user-lisp-directory~ is a user option introduced in Emacs 31. It
makes Emacs byte compile Emacs Lisp files therein and scan them for
autoloads. This is great news because now my custom code is treated
the same way as packages I install ([[#h:e289a614-4f17-4d6c-a028-42fe45aebe66][The =init.el= arrangements for my own modules and custom libraries]]).

#+begin_src emacs-lisp :tangle "early-init.el"
(setq user-lisp-directory (locate-user-emacs-file "minimal-lisp/"))
#+end_src

** COMMENT The =early-init.el= defines general theme-related functions
:PROPERTIES:
...
- [[#h:36af519a-67e1-42b6-a38a-2052638108f9][The =minimal-emacs-modus-themes.el= module]]
- [[#h:2b2a27a1-6d2e-4b59-bf60-94682e173f2f][The =minimal-emacs-ef-themes.el= module]]
- [[#h:334ecb7f-91e0-40db-bd47-418ea0e580e9][The =minimal-emacs-doric-themes.el= module]]
- [[#h:0c7a240c-211f-4571-a5ba-433e510f487f][The =minimal-emacs-standard-themes.el= module]]

#+begin_src emacs-lisp :tangle "early-init.el"
;;;; General theme code

(defun minimal-emacs-theme-gsettings-dark-p ()
...
the `delight.sh' shell script."
  (minimal-emacs-with-desktop-session
    (string-match-p
     "dark"
     (shell-command-to-string "gsettings get org.gnome.desktop.interface color-scheme"))))

(defun minimal-emacs-theme-twm-dark-p ()
  "Return non-nil if my custom setup has a dark theme.
I place a file in ~/.config/minimal-xtwm-active-theme which contains
a single word describing my system-wide theme.  This is part of
my dotfiles.  Check my `delight.sh' shell script for more."
  (when-let* ((file "~/.config/minimal-xtwm-active-theme")
              ((file-exists-p file)))
    (string-match-p
     "dark"
     (with-temp-buffer
...
(defun minimal-emacs-theme-environment-dark-p ()
  "Return non-nil if environment theme is dark."
  (or (minimal-emacs-theme-twm-dark-p)
      (minimal-emacs-theme-gsettings-dark-p)))

(defun minimal-emacs-re-enable-frame-theme (_frame)
  "Re-enable active theme, if any, upon FRAME creation.
Add this to `after-make-frame-functions' so that new frames do
not retain the generic background set by the function
`minimal-emacs-avoid-initial-flash-of-light'."
  (when-let* ((theme (car custom-enabled-themes)))
    (enable-theme theme)))
#+end_src

** COMMENT The =early-init.el= takes care to avoid the initial flash of light
:PROPERTIES:
...
;; `minimal-emacs-theme-gsettings-dark-p' will load a dark theme.
(defun minimal-emacs-avoid-initial-flash-of-light ()
  "Avoid flash of light when starting Emacs, if needed.
New frames are instructed to call `minimal-emacs-re-enable-frame-theme'."
  (when (minimal-emacs-theme-environment-dark-p)
    (setq mode-line-format nil)
    (set-face-attribute 'default nil :background "#000000" :foreground "#ffffff")
    (set-face-attribute 'mode-line nil :background "#000000" :foreground "#ffffff" :box 'unspecified)
    (add-hook 'after-make-frame-functions #'minimal-emacs-re-enable-frame-theme)))
...
([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =minimal-emacs-window.el= section about ~beframe~]]). The multi-frame
...
All user options must be set in a =minimal-emacs-pre-custom.el= file in
...
I normally use ~vertico~ with my own custom extensions to it ([[#h:3796a4c9-8659-4782-8aaa-3cf4e950927d][The =minimal-vertico.el= library]]),
...
- [[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =minimal-emacs-completion.el= for the ~orderless~ completion style]]
- [[#h:bd3f7a1d-a53d-4d3e-860e-25c5b35d8e7e][The =minimal-emacs-completion.el= section to configure completion annotations (~marginalia~)]]
- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =minimal-emacs-completion.el= for in-buffer completion popup (~corfu~)]]
- [[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =minimal-emacs-completion.el= settings for ~consult~]]
- [[#h:61863da4-8739-42ae-a30f-6e9d686e1995][The =minimal-emacs-completion.el= section about ~embark~]]
...
Also check [[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][the =minimal-emacs-which-key.el= module]].
...
Also check [[#h:4efde779-3ef0-40b7-82e4-70f71b321428][the =minimal-emacs-icons.el= module]].
...
surplus to requirements ([[#h:e38f15b2-a849-47c4-9aa8-8e5fcf17f415][The =minimal-emacs-langs.el= settings for ~emacs-lisp-mode~ and ~minimal-elisp~]]).
...
- The =minimal-emacs-modules= directory :: This is where I store all the
  ...
  with =minimal-emacs-=, followed by a word that broadly describes their
  scope of application, such as ~minimal-emacs-font~, ~minimal-emacs-window~...
  ...
  the file name minus its file type extension: ~minimal-emacs-theme~ is
  the feature provided by =minimal-emacs-theme.el=.
...
- The =minimal-lisp= directory :: As with the aforementioned modules,
...
=README.org= file or its web page version. Still, this arrangement
...
As of Emacs 31, the user option ~user-lisp-directory~ can read a
directory and treat it as if it contains actual packages. This means
that it byte compiles it and scans it for autoloads. This is great
news! The ~user-lisp-directory~ is set in the early init, so that it
takes effect before loading the rest of the configuration ([[#h:d4e2145a-0aae-4c8e-9518-4cf2ec1a329a][The =early-init.el= sets the ~user-lisp-directory~]]).
...
use-case. Since the introduction of the =early-init.el= file, we also
do not need to initialise the packages at this point: we activate the
cache instead ([[#h:7a037504-8a2f-4df0-8482-ce6476354440][The =early-init.el= initialises the package cache]]).
...
** The =init.el= macro to do nothing with Elisp code (~minimal-emacs-comment~)
:PROPERTIES:
...
~minimal-emacs-comment~ (though another is needed to also remove it and
...
** The =init.el= macro to install a package (~minimal-emacs-install~)
:PROPERTIES:
:CUSTOM_ID: h:aebcd86e-1f3e-4be5-a2c0-200df2b9e712
:END:

The ~minimal-emacs-install~ macro does what I want to install packages
...
(minimal-emacs-install modus-themes "https://github.com/protesilaos/modus-themes.git")
;; => (unless (package-installed-p 'modus-themes)
...
(minimal-emacs-install modus-themes :url "https://github.com/protesilaos/modus-themes.git" :branch "main")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
;; =>       (apply #'package-vc-install
...
(minimal-emacs-install modus-themes "~/Git/Projects/modus-themes")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
...
(minimal-emacs-install modus-themes "~/Git/Projects/modus-themes-arbitrary-name-of-dir" "modus-themes")
;; => (unless (package-installed-p 'modus-themes)
;; =>   (condition-case-unless-debug err
;; =>       (apply #'package-vc-install
...
** The =init.el= macro to add a hook (~minimal-emacs-hook~)
:PROPERTIES:
:CUSTOM_ID: h:283440db-f5db-4259-9be1-5a2127675824
:END:

The ~minimal-emacs-hook~ is my way to set up hooks. It is flexible enough
...
** The =init.el= macro to bind keys (~minimal-emacs-keybind~)
:PROPERTIES:
...
** The =init.el= macro to add autoloads (~minimal-emacs-autoload~)
:PROPERTIES:
:CUSTOM_ID: h:915cab3f-8cdc-46ab-a608-290992be2cfd
:END:

The ~minimal-emacs-autoload~ will generate autoloads for one or more
functions belonging to a given file. Packages should already handle
this, though I still have a need for it in all the local libraries I
maintain. I could handle those are "packages", in terms of generating
the concomitant autoloads files, but it is easier to simply declare
autoloads in the configuration. They only apply to my setup, anyway.

The macro works like this:

#+begin_example emacs-lisp
(minimal-emacs-autoload test-fn-1 "test-file")
;; => (autoload #'test-fn-1 "test-file")

(minimal-emacs-autoload (test-fn-1 test-fn-2) "test-file")
;; => (progn
;; =>   (autoload #'test-fn-1 "test-file")
;; =>   (autoload #'test-fn-2 "test-file"))
#+end_example
...
** The =init.el= macro to define abbreviations (~minimal-emacs-abbrev~)
:PROPERTIES:
...
This is the same idea as ~minimal-emacs-keybind~, adjusted to work with
the ~define-abbrev~ function ([[#h:d3684ec0-a972-4033-a4d6-41ee16e891d9][The init.el macro to bind keys (~minimal-emacs-keybind~)]]).
...
** The =init.el= macro to configure a package (~minimal-emacs-configure~)
:PROPERTIES:
:CUSTOM_ID: h:e3aa22c1-e074-4bcd-8867-7dbcf1de4a81
:END:

The ~minimal-emacs-configure~ is a simple wrapper around
...
those options must have the =minimal-emacs-pre-custom.el= file in place,
...
** The =minimal-emacs-theme.el= module
:PROPERTIES:
...
*** The =minimal-emacs-theme.el= section to load a theme (~minimal-emacs-load-theme-family~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-theme.el" :mkdirp yes
...
**** The =minimal-emacs-modus-themes.el= module
:PROPERTIES:
:CUSTOM_ID: h:36af519a-67e1-42b6-a38a-2052638108f9
:END:

This is one of the components of the =minimal-emacs-theme.el= module
([[#h:3f1a33e4-762e-4cf3-a0ae-4c2954d9cbb8][The =minimal-emacs-theme.el= section to load a theme (~minimal-emacs-load-theme-family~)]]).
...
Note that the ~minimal-emacs-comment~ is there for my testing purposes
([[#h:3b14faa6-83fd-4d5f-b3bc-85f72fd572d4][The =init.el= macro to do nothing with Elisp code (~minimal-emacs-comment~)]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-modus-themes.el"
;;; The Modus themes
...
**** The =minimal-emacs-solarized-themes.el= module
:PROPERTIES:
:CUSTOM_ID: h:b7923dc9-5e93-4646-be41-396c92253996
:END:

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-solarized-themes.el"
(minimal-emacs-configure
...
**** The =minimal-emacs-ef-themes.el= module
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-ef-themes.el"
;;; The Ef (εὖ) themes
...
**** The =minimal-emacs-doric-themes.el= module
:PROPERTIES:
...
package ([[#h:2b2a27a1-6d2e-4b59-bf60-94682e173f2f][The =minimal-emacs-ef-themes.el= module]]). For something
in-between, which I would consider the best "default theme" for a text
editor, opt for my ~modus-themes~ ([[#h:36af519a-67e1-42b6-a38a-2052638108f9][The =minimal-emacs-modus-themes.el= module]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-doric-themes.el"
;;; The Doric themes
...
**** The =minimal-emacs-standard-themes.el= module
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-standard-themes.el"
;;; The Standard themes
...
*** The =minimal-emacs-theme.el= section for ~pulsar~
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-theme.el"
;;;; Pulsar
...
*** The =minimal-emacs-theme.el= section for ~lin~
:PROPERTIES:
...
*** The =minimal-emacs-theme.el= section for ~spacious-padding~
:PROPERTIES:
...
([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =minimal-emacs-modeline.el= module]]).
...
*** The =minimal-emacs-theme.el= section for ~rainbow-mode~
:PROPERTIES:
...
*** The =minimal-emacs-theme.el= section for ~cursory~
:PROPERTIES:
...
*** The =minimal-emacs-theme.el= section for ~theme-buffet~
:PROPERTIES:
...
*** The =minimal-emacs-theme.el= section about ~fontaine~
:PROPERTIES:
...
([[#h:60d6aae2-6e4b-402c-b6a8-411fc49a6857][The =minimal-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing]]).

Also check my ~show-font~ package ([[#h:60a005be-77bd-49f1-a865-78d7cf75bd2a][The =minimal-emacs-theme.el= section about ~show-font~]]).
...
*** The =minimal-emacs-theme.el= section about ~show-font~
:PROPERTIES:
...
To actually set fonts, I use my ~fontaine~ package ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =minimal-emacs-theme.el= section about ~fontaine~]]).
...
*** The =minimal-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing
:PROPERTIES:
...
families in my ~fontaine~ setup ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =minimal-emacs-theme.el= section about ~fontaine~]]).
...
*** The =minimal-emacs-theme.el= call to ~provide~
:PROPERTIES:
...
** The =minimal-emacs-essentials.el= module
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= block with basic configurations
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-essentials.el" :mkdirp yes
...
*** The =minimal-emacs-essentials.el= section about =minimal-common.el= (custom basic functions)
:PROPERTIES:
:CUSTOM_ID: h:116854fa-2e31-4d23-ae6f-72f57ed571bc
:END:

[[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =minimal-common.el= library]] contains custom snippets that I use in
...
Since the =minimal-common.el= defines some basic functions that can be
...
1. Truncate lines by default in a number of places and do not produce
   ...
   this, i.e. ~minimal-common-truncate-lines-silently~, may also be set
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-essentials.el"
(minimal-emacs-configure
...
*** The =minimal-emacs-essentials.el= section about =minimal-simple.el= (custom basic commands)
:PROPERTIES:
:CUSTOM_ID: h:f15bc843-2dd1-4792-95ec-9b7b6e561804
:END:

[[#h:5f78e837-0d27-4390-bd9a-6d0bca57fa50][The =minimal-simple.el= library]] is done in the same spirit as the
...
The ~minimal-simple-display-unsaved-buffers-on-exit~ produces a list of
...
*** The =minimal-emacs-essentials.el= section about =minimal-scratch.el= (scratch buffer per major mode)
:PROPERTIES:
:CUSTOM_ID: h:45b4fffa-2ce3-4119-b8c9-081a75135d98
:END:

[[#h:efebaec4-7273-4c5a-a4c4-9f99df0e46bb][The =minimal-scratch.el= library]] provides the means to create a scratch
...
buffer. Read the doc string of the command ~minimal-scratch-buffer~.
...
*** The =minimal-emacs-essentials.el= section about =minimal-pair.el= (insert character pairs)
:PROPERTIES:
:CUSTOM_ID: h:48d2bab5-241f-49dd-9554-8a4dcedce75e
:END:

[[#h:0a3f076d-386d-47e6-8af9-f17d02d507cf][The =minimal-pair.el= library]] defines a mechanism to insert character
pairs around the symbol at point or the active region. There is a user
option called ~minimal-pair-pairs~, which specifies which characters form
...
*** The =minimal-emacs-essentials.el= section for comments
:PROPERTIES:
:CUSTOM_ID: h:85755648-bf1f-408b-bc65-4db357fbf1dd
:END:

[[#h:7affcbca-0808-4159-8eab-faa7973aefc7][The =minimal-comment.el= library]] expands the built-in commenting
facilities with what makes sense to me. The ~minimal-comment~ command is
like the built-in ~comment-dwim~, but toggles linewise commenting
instead of appending to them by default. While the ~minimal-comment-timestamp-keyword~
prompts for a keyword among ~minimal-comment-keywords~ and formats it as
a comment with a timestamp next to it. The format of the latter is
controlled by the user option ~minimal-comment-timestamp-format-concise~
or ~minimal-comment-timestamp-format-verbose~ (the verbose is set when
...
default ([[#h:7affcbca-0808-4159-8eab-faa7973aefc7][The =minimal-comment.el= library]]). While I use this section of
...
*** The =minimal-emacs-essentials.el= section about =minimal-prefix.el= (prefix nested keymaps)
:PROPERTIES:
...
[[#h:f860bd85-671e-477e-8233-23a8ba43e1c6][The =minimal-prefix.el= library]] defines a set of keymaps with commonly
...
configurable amount of time ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =minimal-emacs-which-key.el= module]]).
...
*** The =minimal-emacs-essentials.el= configuration to track recently visited files (~recentf-mode~)
:PROPERTIES:
...
completion ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =minimal-emacs-completion.el= module]]).
...
bookmarks ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =minimal-emacs-completion.el= settings for ~consult~]]).
...
*** The =minimal-emacs-essentials.el= mouse configurations and scroll behaviour
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= settings for ~repeat-mode~
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= settings for bookmarks
:PROPERTIES:
...
Also see [[#h:5685df62-4484-42ad-a062-d55ab19022e3][the =minimal-emacs-essentials.el= settings for registers]].
...
*** The =minimal-emacs-essentials.el= settings for registers (~minimal-register~)
:PROPERTIES:
...
quickly ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =minimal-emacs-essentials.el= settings for bookmarks]]). A
...
with ~savehist-mode~ ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =minimal-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).

My =minimal-register.el= library defines a new register method to store
the point in a given file. It also introduces two "Do-What-I-Mean"
commands, one to store a register, and another to load it. Finally, it
defines the ~minimal-register-persist-mode~ which saves the registers to
a file (controlled by ~minimal-register-save-file~) whenever I make a
change to them.
...
*** The =minimal-emacs-essentials.el= settings for auto revert
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section for ~delete-selection-mode~
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= settings for tooltips
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= configurations for the date and time (~display-time-mode~)
:PROPERTIES:
:CUSTOM_ID: h:d4e30bcb-2f90-445f-a5ca-856e2576483a
:END:

I like to display the current date and time on the mode line([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =minimal-emacs-modeline.el= module]]).
...
([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)]]).
...
*** The =minimal-emacs-essentials.el= settings for the ~world-clock~
:PROPERTIES:
...
([[#h:b5fa481d-8549-4424-869e-91091cdf730b][The =minimal-emacs-window.el= module]]).
...
*** The =minimal-emacs-essentials.el= settings for manpages
:PROPERTIES:
:CUSTOM_ID: h:5eb16d63-ce21-4e76-8789-eef6ddbd7c00
:END:

Most buffers conform with rules we define in the ~display-buffer-alist~
([[#h:b5fa481d-8549-4424-869e-91091cdf730b][The =minimal-emacs-window.el= module]]). However, =M-x man= does not do
...
*** The =minimal-emacs-essentials.el= settings for ~proced~
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= arrangement to run Emacs as a server
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section about ~substitute~
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section about ~goto-chg~ (go to change)
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section about ~tmr~ (set timers)
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section about ~password-store~
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= section about generic shells and REPLs (~comint~)
:PROPERTIES:
...
elsewhere ([[#h:7c25e281-d92e-4605-b5d6-18fe1f280a73][The =minimal-emacs-essentials.el= section about ~shell~]]), as
...
([[#h:61c101b0-fd5e-41ba-9031-9236851af696][The =minimal-emacs-essentials.el= section about the compilation interface (~compile~)]]).
...
*** The =minimal-emacs-essentials.el= section about the compilation interface (~compile~)
:PROPERTIES:
:CUSTOM_ID: h:61c101b0-fd5e-41ba-9031-9236851af696
:END:

Similar to the ~comint~ library ([[#h:3ce29c2d-27ce-4f04-86ff-6acaf9426604][The =minimal-emacs-essentials.el= section about generic shells and REPLs (~comint~)]]),
...
(add-to-list 'compilation-error-regexp-alist 'minimal-sample)
#+end_example
...
~project-compile~ ([[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =minimal-emacs-git.el= section about =project.el=]]).
...
*** The =minimal-emacs-essentials.el= section about ~shell~
:PROPERTIES:
...
- [[#h:3ce29c2d-27ce-4f04-86ff-6acaf9426604][The =minimal-emacs-essentials.el= section about generic shells and REPLs (~comint~)]].
- [[#h:61c101b0-fd5e-41ba-9031-9236851af696][The =minimal-emacs-essentials.el= section about the compilation interface (~compile~)]].
...
terminal ([[#h:14d40e5a-327e-453e-861c-3ece24351d62][The =minimal-emacs-essentials.el= settings for ~proced~]]).
...
because my =minimal-shell.el= provides a few niceties that improve the
user experience ([[#h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e][The =minimal-shell.el= library]]).
...
the command ~minimal-shell~: it not only uses a unique and informative
...
My ~minimal-shell-mode~ defines a few extra key bindings (per the
~minimal-shell-mode-map~) and also implements a bookmark handler for
shell buffers ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =minimal-emacs-essentials.el= settings for bookmarks]]).
...
*** The =minimal-emacs-essentials.el= section about the laptop battery (~display-battery-mode~)
:PROPERTIES:
...
*** The =minimal-emacs-essentials.el= call to ~provide~
:PROPERTIES:
...
** The =minimal-emacs-modeline.el= module
:PROPERTIES:
...
([[#h:d4e30bcb-2f90-445f-a5ca-856e2576483a][The =minimal-emacs-essentials.el= configurations for the date and time (~display-time-mode~)]])
or the ~notmuch-indicator~ ([[#h:0f12f144-ca9f-4d07-8563-c33b3521bfb5][The =minimal-emacs-notmuch.el= section about the ~notmuch-indicator~]]).

Most items that go into the ~mode-line-format~ are defined in
[[#h:935adc09-abaa-4413-a5ab-a7a562081c20][the =minimal-modeline.el= library]]. The frame name, in particular, is
useful to me only when I am on the laptop, hence the option to display
it conditionally: ~minimal-modeline-show-frame-name~ ([[#h:0403b581-142b-4613-bc81-51c5dccd831d][The =early-init.el= code to set frame parameters for the desktop or laptop]]).
...
~spacious-padding~ package ([[#h:6c118185-fcb1-4c9a-93af-71814cb84279][The =minimal-emacs-theme.el= section for ~spacious-padding~]]).
It adds, among others, a box effect to mode line constructs.
To make the faces of ~minimal-modeline.el~ look right in this scenario, I
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-modeline.el" :mkdirp yes
;;; Mode line
(minimal-emacs-configure
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-modeline.el"
...
*** The =minimal-emacs-modeline.el= section about ~keycast~
:PROPERTIES:
...
with my customised mode line ([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =minimal-emacs-modeline.el= module]]).
...
*** The =minimal-emacs-modeline.el= call to ~provide~
:PROPERTIES:
...
** The =minimal-emacs-completion.el= module
:PROPERTIES:
...
*** The =minimal-emacs-completion.el= settings for completion styles
:PROPERTIES:
...
of ([[#h:0d49218d-4199-4847-9765-60a3f94e515d][The =minimal-emacs-completion.el= settings for completion category overrides]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-completion.el" :mkdirp yes
...
*** The =minimal-emacs-completion.el= settings for completion category overrides
:PROPERTIES:
:CUSTOM_ID: h:0d49218d-4199-4847-9765-60a3f94e515d
:END:

This builds on the code we have for the ~completion-styles~
([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =minimal-emacs-completion.el= settings for completion styles]]).
...
for those are specified in the ~completion-category-defaults~. The
overrides for them can be set in the ~completion-category-overrides~.
...
annotating, sorting, and grouping candidates. My =minimal-minibuffer.el=
defines several such functions ([[#h:761861c9-9a0e-4c97-b98d-20f13a6b3b79][The =minimal-minibuffer= library]]). It
...
herein. The ~minimal-minibuffer-missing-categories-mode~ is responsible
for this.
...
*** The =minimal-emacs-completion.el= for the ~orderless~ completion style
:PROPERTIES:
:CUSTOM_ID: h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2
:END:

The ~orderless~ package by Omar Antolín Camarena provides one of the
completion styles that I use ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =minimal-emacs-completion.el= settings for completion styles]]).
...
annotations displayed by ~marginalia~ ([[#h:bd3f7a1d-a53d-4d3e-860e-25c5b35d8e7e][The =minimal-emacs-completion.el= section to configure completion annotations (~marginalia~)]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-completion.el"
...
*** The =minimal-emacs-completion.el= settings to ignore letter casing
:PROPERTIES:
...
*** The =minimal-emacs-completion.el= settings for recursive minibuffers
:PROPERTIES:
...
*** The =minimal-emacs-completion.el= settings for default values
:PROPERTIES:
...
*** The =minimal-emacs-completion.el= settings for common interactions
:PROPERTIES:
...
*** The =minimal-emacs-completion.el= generic minibuffer UI settings
:PROPERTIES:
:CUSTOM_ID: h:de61a607-0bdf-462b-94cd-c0898319590e
:END:

These are some settings for the default completion user interface.
These do not come into effect unless ~minimal-emacs-completion-ui~ is set
to a ~nil~ value.

Because I do not like how the default minibuffer user interface works,
I maintain some customisations as part of my =minimal-minibuffer.el= ([[#h:761861c9-9a0e-4c97-b98d-20f13a6b3b79][The =minimal-minibuffer= library]]).
Some of those are about the completion category overrides to add
grouping and sorting facilities ([[#h:0d49218d-4199-4847-9765-60a3f94e515d][The =minimal-emacs-completion.el= settings for completion category overrides]]).
The changes that are specific to the minibuffer interface for
completion are collectively enabled through the minor mode ~minimal-minibuffer-completions-mode~.

- [[#h:f012a254-2716-4c29-a64b-c2b3df34f57f][The =init.el= option to load a minibuffer user interface]]
- [[#h:cff33514-d3ac-4c16-a889-ea39d7346dc5][The =minimal-emacs-completion.el= submodule for ~vertico~ (=minimal-emacs-vertico.el=)]]
- [[#h:e9fb09f0-f2e6-4943-83bf-e5fadcfa530a][The =minimal-emacs-completion.el= submodule for ~mct~ (=minimal-emacs-mct.el=)]]
...
*** The =minimal-emacs-completion.el= settings for saving the history (~savehist-mode~)
:PROPERTIES:
...
- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =minimal-emacs-completion.el= for in-buffer completion popup and preview (~corfu~)]]
- [[#h:5685df62-4484-42ad-a062-d55ab19022e3][The =minimal-emacs-essentials.el= settings for registers]]
- [[#h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e][The =minimal-shell.el= library]]
...
*** The =minimal-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)
:PROPERTIES:
...
static, user-defined abbreviations ([[#h:fd84b79a-351e-40f0-b383-bf520d77834b][The =minimal-emacs-completion.el= settings for static text expansion (~abbrev~)]]).
...
*** The =minimal-emacs-completion.el= settings for static text expansion (~abbrev~)
:PROPERTIES:
...
to what we have specified ([[#h:567bb00f-1d82-4746-93e5-e0f60721728a][The =minimal-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]]).
...
As you will notice below, I use the ~minimal-emacs-abbrev~ macro to make
it easier to define these ([[#h:e7a12825-7848-42bd-b99b-b87903012814][The =init.el= macro to define abbreviations (~minimal-emacs-abbrev~)]]).
...
*** The =minimal-emacs-completion.el= settings for in-buffer completion popup (~corfu~)
:PROPERTIES:
:CUSTOM_ID: h:804b858f-7913-47ef-aaf4-8eef5b59ecb4
:END:

I generally do not rely on in-buffer text completion, no matter how
minimalist it is ([[#h:bb7959b3-d8e6-4163-8e32-bec2b96b7dde][The =minimal-emacs-completion.el= for in-buffer completion hints (~completion-preview-mode~)]]).
...
where the cursor is, if re-indentation cannot be performed there ([[#h:559713c8-0e1e-44aa-bca8-0caae01cc8bb][The =minimal-emacs-langs.el= settings for TAB]]).
...
Also see [[#h:567bb00f-1d82-4746-93e5-e0f60721728a][the =minimal-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]].
...
*** The =minimal-emacs-completion.el= for in-buffer completion hints (~completion-preview-mode~)
:PROPERTIES:
:CUSTOM_ID: h:bb7959b3-d8e6-4163-8e32-bec2b96b7dde
:END:

[ NOTE 2025-12-10: I stopped using this because I find it distracting.
  Same with ~corfu~, by the way ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =minimal-emacs-completion.el= settings for in-buffer completion popup (~corfu~)]]):
...
*** The =minimal-emacs-completion.el= settings for ~consult~
:PROPERTIES:
...
- A case where filtering is in use is the ~consult-buffer~ command,
  ...
  bookmarks ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =minimal-emacs-essentials.el= settings for bookmarks]]),
...
- Asynchronous input pertains to the intersection between Emacs and
  ...
  by wrapping the former inside of =#=. So the input =#minimal-#completion=
  will send =minimal-= to the ~grep~ program and then use =completion=
  inside of the minibuffer to perform the subsequent pattern-matching
  (e.g. with help from ~orderless~ ([[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =minimal-emacs-completion.el= for the ~orderless~ completion style]]).
...
Also check: [[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =minimal-emacs-search.el= module]].
...
*** The =minimal-emacs-completion.el= section about ~embark~
:PROPERTIES:
...
do with all well-behaved buffers ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =minimal-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]).
...
Another downside, which is also true for ~which-key~ ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =minimal-emacs-which-key.el= module]]),
...
Up until 2025-04-10, I maintained a =minimal-embark.el= package as part
...
*** The =minimal-emacs-completion.el= section to configure completion annotations (~marginalia~)
:PROPERTIES:
...
accordingly ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =minimal-emacs-completion.el= settings for completion styles]]).
...
*** The =minimal-emacs-completion.el= setting to load a minibuffer UI submodule
:PROPERTIES:
...
**** The =minimal-emacs-completion.el= submodule for ~mct~ (=minimal-emacs-mct.el=)
:PROPERTIES:
:CUSTOM_ID: h:e9fb09f0-f2e6-4943-83bf-e5fadcfa530a
:END:

[ This is subject to a user option: [[#h:db620e44-fc8f-45b8-a7fd-ae1d0b7ce86c][The =minimal-emacs-completion.el= setting to load a minibuffer UI submodule]]. ]
...
pop up the minibuffer eagerly ([[#h:cff33514-d3ac-4c16-a889-ea39d7346dc5][The =minimal-emacs-completion.el= submodule for ~vertico~ (=minimal-emacs-vertico.el=)]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-mct.el" :mkdirp yes
...
**** The =minimal-emacs-completion.el= submodule for ~vertico~ (=minimal-emacs-vertico.el=)
:PROPERTIES:
...
The extras I provide for ~vertico~ are in [[#h:3796a4c9-8659-4782-8aaa-3cf4e950927d][the =minimal-vertico.el= library]].

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-vertico.el" :mkdirp yes
...
** The =minimal-emacs-search.el= module
:PROPERTIES:
...
- =M-s o= (~occur~) :: Search for the given regular expression
  ...
  extensions: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]].
...
*** The =minimal-emacs-search.el= on isearch lax space
:PROPERTIES:
...
([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =minimal-emacs-search.el= module]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-search.el" :mkdirp yes
...
*** The =minimal-emacs-search.el= settings for isearch highlighting
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-search.el"
(minimal-emacs-configure
...
*** The =minimal-emacs-search.el= on isearch match counter
:PROPERTIES:
...
*** The =minimal-emacs-search.el= tweaks to the isearch motion behaviour
:PROPERTIES:
...
the opposite end with =C-RET= while in an isearch (~minimal-search-isearch-other-end~).

[ See: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]. ]
...
*** The =minimal-emacs-search.el= tweaks for the occur buffer
:PROPERTIES:
:CUSTOM_ID: h:85aca4da-b89b-4fbe-89e9-3ec536ad7b0d
:END:

Here I am making some minor tweaks to =*occur*= buffer (remember to
read the introduction to this section ([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =minimal-emacs-search.el= module]])).
...
The latter depends on my =minimal-common.el= library, which is loaded
early at startup ([[#h:713ede33-3802-40c6-a8e3-7e1fc0d0a924][The =minimal-emacs-essentials.el= block that loads my custom libraries]]).
...
*** The =minimal-emacs-search.el= modified isearch and occur key bindings
:PROPERTIES:
...
*** The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library
:PROPERTIES:
:CUSTOM_ID: h:b902e6a3-cdd2-420f-bc99-3d973c37cd20
:END:

My =minimal-search.el= provides lots of useful extras that I use on a
regular basis ([[#h:0cab525d-fcbc-478c-b914-bbd69e33c635][The =minimal-search.el= library]]).
...
- Place the cursor on the opposite end of an isearch when exiting. Do
  it with =C-RET= while in isearch ([[#h:e6fd4ff0-86c7-423b-9cf3-0a9061505136][The =minimal-emacs-search.el= tweaks to the isearch motion behaviour]]).

- Delete the non-matching portion of a query in isearch with a single
  backspace instead of doing it character-by-character.

- Type =M-s M-<= (~minimal-search-isearch-beginning-of-buffer~) or =M-s
  M->= (~minimal-search-isearch-end-of-buffer~) to search for the symbol
...
where windows/buffers are displayed ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =minimal-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]):

- Type =M-s s= (~minimal-search-outline~) to use minibuffer completion
  ...
  minibuffer user interface ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =minimal-emacs-completion.el= module]]).
  ...
  Consult command on =M-s M-s= ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =minimal-emacs-completion.el= settings for ~consult~]]).

- Type =M-s M-o= (~minimal-search-occur-outline~) to produce an outline
  of the given buffer. What constitutes an outline is defined in the
  user option ~minimal-search-outline-regexp-alist~. I only configure it
  for the major modes I care about, though the mechanism should work
  for every buffer that has an outline.

- Type =M-s M-t= (~minimal-search-grep-todo-keywords~) to produce an
  =*occur*= buffer that matches keywords like =TODO=. The full regular
  expression is specified in the user option ~minimal-search-todo-keywords~.

- Type =C-u M-s M-t= or =M-s M-T= (~minimal-search-git-grep-todo-keywords~)
  ...
  ([[#h:9a3581df-ab18-4266-815e-2edd7f7e4852][The =minimal-emacs-search.el= setup for editable grep buffers (~wgrep~)]]).

- Type =M-s g= (~minimal-search-grep~) to perform a "local grep" across
...
matching ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =minimal-emacs-completion.el= settings for ~consult~]]).
...
*** The =minimal-emacs-search.el= tweaks to ~xref~, ~re-builder~ and ~grep~
:PROPERTIES:
...
*** The =minimal-emacs-search.el= setup for editable grep buffers (~grep-edit-mode~ or ~wgrep~)
:PROPERTIES:
...
([[#h:1b53bc10-8b1b-4f68-bbec-165909761e43][The =minimal-emacs-dired.el= section about ~wdired~ (writable Dired)]]).
I use this in tandem with my ~minimal-search-grep~ command.
...
*** The =minimal-emacs-search.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
** The =minimal-emacs-dired.el= module
:PROPERTIES:
...
*** The =minimal-emacs-dired.el= settings for common operations
:PROPERTIES:
...
([[#h:2e005bd1-d098-426d-91f9-2a31a6e55caa][The =minimal-emacs-dired.el= section about =trashed.el=]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-dired.el" :mkdirp yes
...
*** The =minimal-emacs-dired.el= switches for ~ls~ (how files are listed)
:PROPERTIES:
:CUSTOM_ID: h:679e4460-b306-450f-aa20-497243057e02
:END:

As I already explained, Dired is a layer of interactivity on top of
standard Unix tools ([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =minimal-emacs-dired.el= module]]). We can see
...
- =-h= :: Make file sizes easier to read, such as =555k= instead of
  =568024= (the size of =README.org= as of this writing).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-dired.el"
(setq dired-listing-switches "-AGFhlv --group-directories-first --time-style=long-iso")
#+end_src

*** The =minimal-emacs-dired.el= setting for dual-pane Dired
:PROPERTIES:
...
history ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =minimal-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-dired.el"
(setq dired-dwim-target t)
#+end_src

*** The =minimal-emacs-dired.el= settings to open files externally
:PROPERTIES:
...
*** The =minimal-emacs-dired.el= miscellaneous tweaks
:PROPERTIES:
...
([[#h:679e4460-b306-450f-aa20-497243057e02][The =minimal-emacs-dired.el= switches for ~ls~ (how files are listed)]]).
...
([[#h:fbe6f9da-25ee-46a3-bb03-8fa7c1d48dab][The =minimal-emacs-essentials.el= settings for ~repeat-mode~]]).
...
*** The =minimal-emacs-dired.el= section about various conveniences
:PROPERTIES:
...
- the key binding for ~dired-do-open~, which opens the file or
  directory externally ([[#h:d7d5e619-3d50-4d9e-a951-7262462a60c9][The =minimal-emacs-dired.el= settings to open files externally]]).
...
*** The =minimal-emacs-dired.el= section about my extras (=minimal-dired.el=)
:PROPERTIES:
...
actions ([[#h:92cfefcf-8c20-43cd-b47c-ffe0c5fe1887][The =minimal-dired.el= library]]). Chief among them is the
~minimal-dired-limit-regexp~ (bound to =C-c C-l=), which is an easier way
...
All this is fine, but with ~minimal-dired-limit-regexp~ I simply provide
...
current directory. I do this with ~minimal-dired-search-flat-list~.

The other commands have situational uses. For example, the
~minimal-dired-grep-marked-files~ is something I have only used a few
times where ~minimal-search-grep~ would produce too many results in a
given directory ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]).
...
*** The =minimal-emacs-dired.el= section about ~dired-subtree~
:PROPERTIES:
...
*** The =minimal-emacs-dired.el= section about ~wdired~ (writable Dired)
:PROPERTIES:
:CUSTOM_ID: h:1b53bc10-8b1b-4f68-bbec-165909761e43
:END:

As noted in the introduction, Dired can be made writable
([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =minimal-emacs-dired.el= module]]). This way, we can quickly rename
...
*** The =minimal-emacs-dired.el= section about ~image-dired~
:PROPERTIES:
...
*** The =minimal-emacs-dired.el= section about ~dired-preview~
:PROPERTIES:
...
*** The =minimal-emacs-dired.el= section about =trashed.el=
:PROPERTIES:
...
*** COMMENT The =minimal-emacs-dired.el= section about ~mandoura~ (~mpv~ media player)
:PROPERTIES:
...
(minimal-emacs-configure
  (minimal-emacs-install mandoura "https://github.com/protesilaos/mandoura")
  (with-eval-after-load 'dired
    (require 'mandoura)
    (minimal-emacs-keybind global-map
...
*** The =minimal-emacs-dired.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
** The =minimal-emacs-window.el= module
:PROPERTIES:
...
*** The =minimal-emacs-window.el= section about running commands in popup frames
:PROPERTIES:
...
Emacs daemon process or ~server-mode~ ([[#h:7709b7e9-844f-49f3-badf-784aacec4bca][The =minimal-emacs-essentials.el= arrangement to run Emacs as a server]]).
We can thus call ~emacsclient~ from outside Emacs and instruct it to
run any arbitrary Emacs Lisp code.

My =minimal-window.el= provides the ~minimal-window-define-with-popup-frame~
macro which streamlines the creation of commands that have the desired
behaviour ([[#h:35b8a0a5-c447-4301-a404-bc274596238d][The =minimal-window.el= library]]). All we need is to call the
...
(with-eval-after-load 'org-capture
  (add-hook 'org-capture-after-finalize-hook #'minimal-window-delete-popup-frame))

(with-eval-after-load 'tmr
  (add-hook 'tmr-timer-created-functions #'minimal-window-delete-popup-frame))

(with-eval-after-load 'minimal-project
  (add-hook 'minimal-project-switch-hook #'minimal-window-delete-popup-frame))
#+end_example

The commands I configure are for ~org-capture~ and ~tmr~ as well as my
custom ~minimal-project-switch~:

- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:6dce278e-86c7-4620-9271-d5af5499f0cc][The =minimal-emacs-essentials.el= section about ~tmr~ (set timers)]]
- [[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =minimal-emacs-git.el= section about =project.el=]]
...
emacsclient -e '(minimal-window-popup-org-capture)'

# Same idea for the `tmr` command.
emacsclient -e '(minimal-window-popup-tmr)'

# Same idea for the `minimal-project-switch` command.
emacsclient -e '(minimal-window-popup-minimal-project-switch)'
#+end_example

*** The =minimal-emacs-window.el= section about the ~tab-bar-mode~
:PROPERTIES:
...
laptop, whose screen real estate is limited ([[#h:080aa291-95b4-4d54-8783-d156b13190e9][The =minimal-emacs-essentials.el= section about the laptop-specific settings]]).
...
custom code I have ([[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =minimal-emacs-git.el= section about =project.el=]]).
...
list of buffers ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =minimal-emacs-window.el= section about ~beframe~]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-window.el" :mkdirp "yes"
(minimal-emacs-configure
...
*** The =minimal-emacs-window.el= section about uniquifying buffer names
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-window.el"
...
*** The =minimal-emacs-window.el= section about line highlighting (~hl-line-mode~)
:PROPERTIES:
...
*** The =minimal-emacs-window.el= section about negative space highlighting (~whitespace-mode~)
:PROPERTIES:
:CUSTOM_ID: h:ad2945c5-992d-4c82-a8b8-652f47ebdbea
:END:

Much like ~hl-line-mode~, I normally do not use the ~whitespace-mode~
([[#h:ec156866-3d25-42e7-8247-b190103b6f17][The =minimal-emacs-window.el= section about line highlighting (~hl-line-mode~)]]).
...
*** The =minimal-emacs-window.el= section about line numbers (~display-line-numbers-mode~)
:PROPERTIES:
...
*** The =minimal-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)
:PROPERTIES:
...
In my =minimal-window.el= library, I define functions that determine how
a buffer should be displayed, given size considerations ([[#h:35b8a0a5-c447-4301-a404-bc274596238d][The =minimal-window.el= library]]).
You will find the functions ~minimal-window-shell-or-term-p~ to determine
what a shell or terminal is, ~minimal-window-display-buffer-below-or-pop~
to display the buffer below the current one or to its side depending
on how much width is available, and ~minimal-window-select-fit-size~ to
...
height or width. These are relevant for ~minimal-window-display-buffer-below-or-pop~
...
*** The =minimal-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts
:PROPERTIES:
...
*** The =minimal-emacs-window.el= settings to truncate some buffers silently
:PROPERTIES:
:CUSTOM_ID: h:b6754236-3d08-4448-9c86-3e5923737e4b
:END:

This here is the opposite of what we saw above ([[#h:bf3f50d5-c5db-4704-9498-9e8bfe1b45a8][The =minimal-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts]]).
...
truncation without displaying a message about the fact ([[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =minimal-common.el= library]]).
...
*** The =minimal-emacs-window.el= section key bindings
:PROPERTIES:
...
*** The =minimal-emacs-window.el= section about ~beframe~
:PROPERTIES:
:CUSTOM_ID: h:77e4f174-0c86-460d-8a54-47545f922ae9
:END:

[ Also see: [[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =minimal-emacs-git.el= section about =project.el=]]. ]
...
screen real estate ([[#h:080aa291-95b4-4d54-8783-d156b13190e9][The =minimal-emacs-essentials.el= section about the laptop-specific settings]]).
...
*** The =minimal-emacs-window.el= configuration of ~undelete-frame-mode~ and ~winner-mode~
:PROPERTIES:
:CUSTOM_ID: h:2df15080-77f9-45f8-a3b5-1adddc70a512
:END:

Since I am using my ~beframe~ package to isolate buffers per frame
([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =minimal-emacs-window.el= section about ~beframe~]]), I appreciate the
...
*** The =minimal-emacs-window.el= keys for window motions (~windmove~)
:PROPERTIES:
...
*** The =minimal-emacs-window.el= ~provide~ form
:PROPERTIES:
...
** The =minimal-emacs-git.el= module
:PROPERTIES:
...
*** The =minimal-emacs-git.el= section about =project.el=
:PROPERTIES:
...
built-in =vc.el= ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =minimal-emacs-git.el= section about =vc.el= and related]]).
...
limited to that frame ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =minimal-emacs-window.el= section about ~beframe~]]).
This does not happen when I am on the laptop though. There I prefer a
singular frame where each projects occupies its own ~tab-bar-mode~ tab ([[#h:b4e65201-3f12-4b24-acb3-a5db48cadb3f][The =minimal-emacs-window.el= section about the ~tab-bar-mode~]]).

Note that [[#h:654cfe49-f59d-4a0d-84ee-094ddce06848][the =minimal-project.el= library]] defines some of the
functionality I rely on, so do take a look at what it has to offer.

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-git.el"
;;;; `project'
(minimal-emacs-configure
  (minimal-emacs-keybind global-map
    "C-x p ." #'project-dired
...
*** The =minimal-emacs-git.el= section about ~diff-mode~
:PROPERTIES:
...
example, fold the invidual diff hunks and move between them ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =minimal-emacs-langs.el= settings for ~outline-minor-mode~]]).
Personally, I combine this feature with my ~minimal-search-outline~
command to quickly jump to an outline heading using minibuffer
completion ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]).
...
*** The =minimal-emacs-git.el= section about ediff
:PROPERTIES:
...
integrates with ~magit~ ([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =minimal-emacs-git.el= section about ~magit~ (great Git client)]]).

The =minimal-ediff.el= extension contains some quality-of-life
refinements to the core Ediff functionality ([[#h:3937b784-af0f-47c0-8c89-c74d90b00b8c][The =minimal-ediff.el= library]]):

- Function ~minimal-ediff-store-layout~ :: Store the current frame's window configuration before Ediff is set up.
- Function ~minimal-ediff-restore-layout~ :: Restore the current frame's window configuration.
- Command ~minimal-ediff-buffers-2~ :: Produce an Ediff with the current two visible buffers.
- Command ~minimal-ediff-buffers-3~ :: Produce an Ediff with the current three visible buffers.

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-git.el" :mkdirp yes
;;;; `ediff'
(minimal-emacs-configure
...
*** The =minimal-emacs-git.el= section about ~smerge-mode~
:PROPERTIES:
...
- [[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =minimal-emacs-git.el= section about =vc.el= and related]].
- [[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =minimal-emacs-git.el= section about ~magit~ (great Git interface)]].
...
*** The =minimal-emacs-git.el= section about =vc.el= and related
:PROPERTIES:
...
([[#h:8b426a69-e3cd-42ac-8788-f41f6629f879][The =minimal-emacs-git.el= section about ~diff-mode~]]).
...
- Merge conflicts are handled in the affected files with the help of
  the built-in ~smerge-mode~ ([[#h:2ed18433-8b38-4518-a464-77bc76e3a87d][The =minimal-emacs-git.el= section about ~smerge-mode~]]).
...
([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =minimal-emacs-git.el= section about ~magit~ (great Git client)]]).
...
*** COMMENT The =minimal-emacs-git.el= section about ~agitate~
:PROPERTIES:
...
functionality provided by the =vc= suite of tools ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =minimal-emacs-git.el= section about =vc.el= and related]]),
as well as that of ~diff-mode~ ([[#h:8b426a69-e3cd-42ac-8788-f41f6629f879][The =minimal-emacs-git.el= section about ~diff-mode~]]).
...
(minimal-emacs-configure
  ...
  (minimal-emacs-keybind diff-mode-map
  ...
  (minimal-emacs-keybind vc-git-log-view-mode-map
...
*** The =minimal-emacs-git.el= section about ~magit~ (great Git interface)
:PROPERTIES:
...
all we need to select targets with ease ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =minimal-emacs-completion.el= module]]).
...
- [[#h:89edea05-4d94-4ea1-b2a8-5ad01422618c][The =minimal-emacs-git.el= section about ediff]].
- [[#h:2ed18433-8b38-4518-a464-77bc76e3a87d][The =minimal-emacs-git.el= section about ~smerge-mode~]].
...
*** The =minimal-emacs-git.el= call to ~provide~
:PROPERTIES:
...
** The =minimal-emacs-org.el= module
:PROPERTIES:
...
- Link to files regardless of file type. This includes special links
  ...
  locally and integrated with Emacs ([[#h:755e195b-9471-48c7-963b-33055969b4e2][The =minimal-emacs-email.el= module]]).
...
*** The =minimal-emacs-org.el= section on the ~calendar~
:PROPERTIES:
...
([[#h:5812a834-f294-4714-85a5-f8466ececaa8][The =minimal-emacs-langs.el= extension for Denote Journal extras (~denote-journal~)]]).

Note that Emacs also has a ~diary~ command. I used it for a while, but
Org is far more capable, so I switched to it completely.

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-org.el" :mkdirp yes
;;; Calendar
(minimal-emacs-configure
...
*** The =minimal-emacs-org.el= section about appointment reminders (=appt.el=)
:PROPERTIES:
...
([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =minimal-emacs-org.el= Org agenda settings]]). I deepen this
...
done at a specific time, such as coaching sessions ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates (~org-capture~)]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-org.el"
...
*** The =minimal-emacs-org.el= section with basic Org settings
:PROPERTIES:
...
*** The =minimal-emacs-org.el= indent and default folding settings for Org
:PROPERTIES:
...
We can set up this minor mode via a hook, so it takes effect in all
Org buffers. An alternative is to enable on a per-file basis, by add
the =indent= word to the =#+startup= keyword somewhere close to the
...
=M-x org-mode-restart= for changes to take effect, or move the cursor
to that line and type =C-c C-c=. Below I configure ~org-startup-folded~
which is underlying variable for the =content= part shown in this example.
...
*** The =minimal-emacs-org.el= Org to-do and refile settings
:PROPERTIES:
...
which I consider helpful ([[#h:0884658e-9eb5-47e3-9338-66e09004a1a0][The =minimal-emacs-org.el= Org time/state logging]]).
...
date ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =minimal-emacs-org.el= Org agenda settings]]).
...
the capture phase ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates (~org-capture~)]]).
...
*** The =minimal-emacs-org.el= Org heading tags
:PROPERTIES:
...
- [[#h:024dd541-0061-4a10-b10b-b17dcd4794b9][The =minimal-emacs-org.el= Org to-do and refile settings]]
- [[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =minimal-emacs-org.el= Org agenda settings]]
...
*** The =minimal-emacs-org.el= Org time/state logging
:PROPERTIES:
...
appointment ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =minimal-emacs-org.el= Org agenda settings]]). This data
...
*** The =minimal-emacs-org.el= Org link settings
:PROPERTIES:
...
- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:49890997-448e-408d-bebe-2003259bb125][The =minimal-emacs-notmuch.el= glue code for ~org-capture~ (=ol-notmuch.el=)]]
...
[[#h:e03df1e0-b43e-49b5-978e-6a511165617c][The =minimal-emacs-org.el= section with basic Org settings]].
...
*** The =minimal-emacs-org.el= Org code block settings
:PROPERTIES:
...
- =#+BEGIN_EXPORT= :: Evaluate the code for the given export target
  ...
  or keeping both of them ([[#h:bd11d4d8-6e9f-4536-87a4-4018783bf8f5][The =minimal-emacs-org.el= Org export settings]]).
...
*** The =minimal-emacs-org.el= Org export settings
:PROPERTIES:
...
I showed for Org Babel ([[#h:1f5a0d46-5202-48dd-8048-b48ce17f3df8][The =minimal-emacs-org.el= Org code block settings]]).
...
*** The =minimal-emacs-org.el= Org capture templates (~org-capture~)
:PROPERTIES:
...
where we can, for example, note the date the entry was recorded,
...
subheadings with the relevant tasks. You will notice how most of my
...
with the general purpose command ~minimal-search-outline~
([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]).
...
- A task is either time-sensitive or not. What is not time-sensitive
  goes into the "Wishlist". Whenever I have time, I will check the
  wishlist to see if there is something I feel is worth trying.

- Time-sensitive tasks are those which must be done in a timely
  fashion: they have inherent timeliness. I do not set aspirational
  deadlines, such as "I hope to do this tomorrow morning, but actually
  I could wait another month". If something has a timestamp, it must
  be done then.

- I use =DEADLINE= to signify the highest time-sensitivity, such as a
  video call with someone: it must start exactly when the timestamp
  says. I use =SCHEDULED= if something is expected to happen at some
  point during the day, but there is no fixed hour, such as the
  delivery of a package. Granted, we can think of the semantics of
  =DEADLINE= and =SCHEDULED= differently, though what ultimately
  matters is to stick to something and be consistent.

- To help me find my tasks more easily, I rely on tags. Those are
  typically inherited from a parent heading or, in the case of
  coaching, the file's =#+filetags=. For example, all meetings are
  nested under a =Meetings= heading with a tag of the same name and
  then in-person appointments are under a different heading than video
  calls, again with their distinct tags. The agenda views help me find
  everything ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =minimal-emacs-org.el= Org agenda settings]]).
...
*** The =minimal-emacs-org.el= Org agenda settings
:PROPERTIES:
...
task in the usual place ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates (~org-capture~)]]).
...
this order ([[#h:9974eac8-2167-45c4-90e0-12dd877403da][The =minimal-org.el= library]]).:
...
*** The =minimal-emacs-org.el= call to ~provide~
:PROPERTIES:
...
** The =minimal-emacs-langs.el= module
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for TAB
:PROPERTIES:
...
completion at point ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =minimal-emacs-completion.el= for in-buffer completion popup (~corfu~)]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-langs.el" :mkdirp yes
...
*** The =minimal-emacs-langs.el= settings for ~emacs-lisp-mode~ and ~minimal-elisp~
:PROPERTIES:
...
Then I set up my ~minimal-elisp~ library ([[#h:9d63bbb2-1ee4-44aa-8601-8728f9f857ec][The =minimal-elisp.el= library]]).
...
The ~minimal-elisp-pp-last-sexp~ is a variant of ~pp-macroexpand-last-sexp~,
whose primary goal is to conform with the ~display-buffer-alist~
([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =minimal-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]).
I can thus macroexpand with the confidence that the resulting buffer
will not mess up with my work.

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-langs.el"
...
*** The =minimal-emacs-langs.el= settings for "electric" behaviour
:PROPERTIES:
...
of characters, I use my ~minimal-pair-insert~ command ([[#h:48d2bab5-241f-49dd-9554-8a4dcedce75e][The =minimal-emacs-essentials.el= section about =minimal-pair.el= (insert character pairs)]]).
...
*** The =minimal-emacs-langs.el= settings ~show-paren-mode~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for plain text (no double spaces, ~auto-fill-mode~)
:PROPERTIES:
...
which corresponds to the ~minimal-simple-unfill-region-or-paragraph~
command ([[#h:f15bc843-2dd1-4792-95ec-9b7b6e561804][The =minimal-emacs-essentials.el= section about =minimal-simple.el= (custom basic commands)]]).
...
*** The =minimal-emacs-langs.el= settings for common file types
:PROPERTIES:
:CUSTOM_ID: h:1a132701-9487-4af0-8759-a8d64d02bf1f
:END:

As I explained above about ~auto-mode-alist~ ([[#h:c5fbd6dd-2982-4fca-aeed-d0e750d399bd][The =minimal-emacs-langs.el= settings for plain text]]),
...
*** The =minimal-emacs-langs.el= settings for ~eldoc~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~eglot~ (LSP client)
:PROPERTIES:
...
- Code linting :: To display suggestions, warnings, or errors. These
  ...
  ([[#h:df6d1b52-0306-4ace-9099-17dded11fbed][The =minimal-emacs-langs.el= settings for code linting (~flymake~)]]).
...
- Code navigation and cross-referencing :: While over a symbol, use a
  ...
  [ Features such as the definition of the outline should, in
    ...
    folding capabilties for outline headings ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =minimal-emacs-langs.el= settings for ~outline-minor-mode~]]).
    I usually navigate the outline using minibuffer completion, with
    the help of my ~minimal-search-outline~ command ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]). ]
...
*** The =minimal-emacs-langs.el= settings for very long lines
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~markdown-mode~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~csv-mode~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~sxhkdrc-mode~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for spell checking
:PROPERTIES:
:CUSTOM_ID: h:115806c4-88b0-43c1-8db2-d9d8d20a5c17
:END:

For spell checking on-demand, I rely on the built-in ~flyspell~
feature that I complement with some small extras ([[#h:551718b7-fb2d-4a4a-804e-3118001b96ec][The =minimal-spell.el= library]]).
...
*** The =minimal-emacs-langs.el= settings for code linting (~flymake~)
:PROPERTIES:
...
LSP linter output ([[#h:92258aa8-0d8c-4c12-91b4-5f44420435ce][The =minimal-emacs-langs.el= settings for ~eglot~]]).
...
*** The =minimal-emacs-langs.el= settings for ~outline-minor-mode~
:PROPERTIES:
...
~outline-minor-mode~, courtesy of the command ~minimal-search-outline~
([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =minimal-emacs-search.el= extras provided by the =minimal-search.el= library]]).
...
*** The =minimal-emacs-langs.el= settings for ~dictionary~
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~altcaps~ (alternating letter casing)
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~denote~ (notes and file-naming)
:PROPERTIES:
...
**** The =minimal-emacs-langs.el= integration between Consult and Denote (~consult-denote~)
:PROPERTIES:
:CUSTOM_ID: h:ee82e629-fb05-4c75-9175-48a760a25691
:END:

This is another package of mine which extends my ~denote~ package
([[#h:e86a66dc-7ef9-4f09-ad7e-946de2034e8d][The =minimal-emacs-langs.el= settings for ~denote~ (notes and file-naming)]]).

This is glue code to integrate ~denote~ with Daniel Mendler's
~consult~ ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =minimal-emacs-completion.el= settings for ~consult~]]). The
...
**** The =minimal-emacs-langs.el= extension for Denote Org extras (~denote-org~)
:PROPERTIES:
...
**** The =minimal-emacs-langs.el= extension for Denote sequence notes or folgezettel (~denote-sequence~)
:PROPERTIES:
...
**** The =minimal-emacs-langs.el= extension for Denote Markdown extras (~denote-markdown~)
:PROPERTIES:
...
**** The =minimal-emacs-langs.el= extension for Denote Journal extras (~denote-journal~)
:PROPERTIES:
...
journal entry for the current date or create a new entry ([[#h:94d48381-1711-4d6b-8449-918bc1e3836c][The =minimal-emacs-org.el= section on the ~calendar~]]).
...
**** The =minimal-emacs-langs.el= extension for Denote Silo extras (~denote-silo~)
:PROPERTIES:
...
*** The =minimal-emacs-langs.el= settings for ~logos~ (writing extras and buffer navigation)
:PROPERTIES:
...
** COMMENT The =minimal-emacs-email.el= module
:PROPERTIES:
...
agenda and has a link back to the original email ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =minimal-emacs-org.el= Org capture templates]]).
...
*** COMMENT The =minimal-emacs-email.el= basic settings (including =authinfo=)
:PROPERTIES:
...
or logging in to IRC with the ~rcirc~ command ([[#h:f23d9cef-ab7b-4486-a070-9c7ae664eadf][The =minimal-emacs-web.el= module]]).
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-email.el" :mkdirp yes
...
*** COMMENT The =minimal-emacs-email.el= message composition and encryption settings (=message.el=)
:PROPERTIES:
...
attach them from there ([[#h:deccd4f1-c1bd-4b02-993e-914f57ea0fac][The =minimal-emacs-email.el= integration with Dired for email attachments (~gnus-dired-mode~)]])
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-email.el"
...
*** COMMENT The =minimal-emacs-email.el= integration with Dired for email attachments (~gnus-dired-mode~)
:PROPERTIES:
...
composition buffer ([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =minimal-emacs-dired.el= module]]). Do it by typing
...
([[#h:49815c07-737f-4f10-95ee-63640f9ae18b][The =minimal-emacs-email.el= message composition and encryption settings (=message.el=)]]).
...
*** COMMENT The =minimal-emacs-email.el= settings for ~sendmail~ (SMTP)
:PROPERTIES:
...
*** COMMENT The =minimal-emacs-email.el= loading of the email client and call to ~provide~
:PROPERTIES:
...
enough ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)]]).
...
*** COMMENT The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-notmuch.el" :mkdirp yes
...
**** COMMENT The =minimal-emacs-notmuch.el= section about the account settings
:PROPERTIES:
:CUSTOM_ID: h:2fca749c-a84e-415c-9ac0-b1518d06fa51
:END:

To actually use the ~notmuch~ mail user agent ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)]]),
...
Notice that I use the ~minimal-common-auth-get-field~ function, which
reads data from my encrypted =~/.authinfo.gpg= file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =minimal-emacs-email.el= basic settings (including =authinfo=)]]).
In =minimal-common.el=, I define a few helper functions that are general
in nature and may be used by other Elisp snippets of mine ([[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =minimal-common.el= library]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= section about the general user interface
:PROPERTIES:
:CUSTOM_ID: h:2c9a555f-d3ec-4d0e-bad6-6c26b576a49a
:END:

When we use the ~notmuch~ Emacs command to get into the mail user
agent ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)]]),
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-notmuch.el"
;;;; General UI
(minimal-emacs-configure
...
**** COMMENT The =minimal-emacs-notmuch.el= section about the presentation of search buffers
:PROPERTIES:
...
**** COMMENT The =minimal-emacs-notmuch.el= section about tag settings
:PROPERTIES:
...
=minimal-notmuch.el= ([[#h:dbd2b775-8f51-451a-8e24-9c8424fd2e89][The =minimal-emacs-notmuch.el= custom extensions (per =minimal-notmuch.el=)]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= section about email composition settings
:PROPERTIES:
...
=message.el= ([[#h:49815c07-737f-4f10-95ee-63640f9ae18b][The =minimal-emacs-email.el= message composition and encryption settings (=message.el=)]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= section about reading messages
:PROPERTIES:
...
the built-in Simple HTML Renderer ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =minimal-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= section about hooks and key bindings
:PROPERTIES:
...
- Remind me when I am mentioning an attachment but do not include one.
  ...
  the ~notmuch-mua-attachment-regexp~ ([[#h:b91c062d-add7-42e6-b425-4fa287763b1b][The =minimal-emacs-notmuch.el= section about email composition settings]]).
...
- Do not activate the ~notmuch-hl-line-mode~ because I want the
  ...
  action ([[#h:bf5b4d08-8f33-4a8c-8ecd-fca19bf2497a][The =minimal-emacs-theme.el= section for ~lin~]]).
...
- Define key bindings that make sense to me. The most important change
  ...
  in the section about my custom extensions for Notmuch ([[#h:dbd2b775-8f51-451a-8e24-9c8424fd2e89][The =minimal-emacs-notmuch.el= custom extensions (per =minimal-notmuch.el=)]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= custom extensions (per =minimal-notmuch.el=)
:PROPERTIES:
:CUSTOM_ID: h:dbd2b775-8f51-451a-8e24-9c8424fd2e89
:END:

My =minimal-notmuch.el= extension defines a few useful extras for my
email setup ([[#h:92ee86b8-0ead-4080-963f-3fbc9f931e3e][The =minimal-notmuch.el= library]]). These are commands to
...
those that are for the base Notmuch package ([[#h:6148eea6-7859-4b53-95ce-7e198fc831e0][The =minimal-emacs-notmuch.el= section about hooks and key bindings]]).
...
**** COMMENT The =minimal-emacs-notmuch.el= glue code for ~org-capture~ (=ol-notmuch.el=)
:PROPERTIES:
...
**** COMMENT The =minimal-emacs-notmuch.el= section about the ~notmuch-indicator~
:PROPERTIES:
...
**** COMMENT The =minimal-emacs-notmuch.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:340e745d-544c-4473-969e-ade21ae7710d
:END:

Finally, we ~provide~ the submodule. This is the mirror function of
~require~ ([[#h:200bc618-678a-4dc8-9d30-876d89e1edae][The =minimal-emacs-email.el= loading of the email client and call to ~provide~]]).

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-notmuch.el"
(provide 'minimal-emacs-notmuch)
#+end_src

*** COMMENT The deprecated =minimal-emacs-mail.el= submodule for ~mu4e~ (=minimal-emacs-mu4e.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-mu4e.el" :mkdirp yes
...
*** COMMENT The deprecated =minimal-emacs-mail.el= submodule for Gnus (=minimal-emacs-gnus.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-gnus.el" :mkdirp yes
...
*** COMMENT Overview of my email setup (~mbsync~, ~msmtp~, mail indexer, and MUA)
:PROPERTIES:
...
as ~mu4e~ ([[#h:97282c11-475c-4ad3-9399-cb2cd12f93b7][The deprecated =minimal-emacs-mail.el= submodule for ~mu4e~ (=minimal-emacs-mu4e.el=)]]), while the
latter's is ~notmuch~ or =notmuch.el= to differentiate it from the
system program ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =minimal-emacs-email.el= submodule for ~notmuch~ (=minimal-emacs-notmuch.el=)]]).
...
implements, which does not rely on ~msmtp~ ([[#h:c7c9815c-e259-4810-816a-9ebc1a146004][The =minimal-emacs-email.el= settings for ~sendmail~ (SMTP)]]).
...
daily computing with the rest of what I do ([[#h:755e195b-9471-48c7-963b-33055969b4e2][The =minimal-emacs-email.el= module]]).

**** COMMENT Contents of my =.mbsyncrc=
:PROPERTIES:
...
file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =minimal-emacs-email.el= basic settings (including =authinfo=)]]).
...
**** COMMENT Contents of my ~msmtp~ configuration file
:PROPERTIES:
...
** The =minimal-emacs-web.el= module
:PROPERTIES:
:CUSTOM_ID: h:f23d9cef-ab7b-4486-a070-9c7ae664eadf
:END:

*** The =minimal-emacs-web.el= settings about following links (~browse-url~)
:PROPERTIES:
...
browser ([[#h:b09b4fce-31fe-4381-a49a-6a7206ac5f1b][The =minimal-emacs-web.el= settings about the web browser (~eww~)]]).
Emacs it can render HTML well, although it does not handle CSS and
JavaScript ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =minimal-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
- [[#h:755e195b-9471-48c7-963b-33055969b4e2][The =minimal-emacs-email.el= module]]
- [[#h:adc258d3-f444-4bb5-8a8d-43025a689167][The =minimal-emacs-web.el= RSS/Atom reader (~elfeed~)]]
- [[#h:b9e6834b-e636-414e-8c50-9e04ba77a0a3][The =minimal-emacs-web.el= settings for the IRC client]]

#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-web.el" :mkdirp yes
;;;; `browse-url'
(minimal-emacs-configure
...
*** The =minimal-emacs-web.el= settings about buttonising links (~goto-address-mode~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-web.el"
;;;; `goto-addr'
(minimal-emacs-configure
...
*** The =minimal-emacs-web.el= settings about the Simple HTML Renderer (~shr~)
:PROPERTIES:
...
- [[#h:edb70963-d590-4959-bc59-1e5435c8e78a][The =minimal-emacs-web.el= settings about following links (~browse-url~)]]
- [[#h:7639683b-4b6f-42ee-9ae0-76eeec6fee9a][The =minimal-emacs-web.el= settings about buttonising links (~goto-addr~)]]
- [[#h:b09b4fce-31fe-4381-a49a-6a7206ac5f1b][The =minimal-emacs-web.el= settings about the web browser (~eww~)]]
...
buffer (as I do, for example, via the ~text-mode-hook~ ([[#h:60d6aae2-6e4b-402c-b6a8-411fc49a6857][The =minimal-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing]])).
...
*** The =minimal-emacs-web.el= settings about browser cookies
:PROPERTIES:
...
*** The =minimal-emacs-web.el= settings about the web browser (~eww~)
:PROPERTIES:
...
page with ~shr~ ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =minimal-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
like Firefox ([[#h:edb70963-d590-4959-bc59-1e5435c8e78a][The =minimal-emacs-web.el= settings about following links (~browse-url~)]]).
...
*** The =minimal-emacs-web.el= extras for ~eww~ (=minimal-eww.el=)
:PROPERTIES:
:CUSTOM_ID: h:1fcd7008-b7ed-4441-8a7b-82db43a05775
:END:

My =minimal-eww.el= used to contain lots of tweaks for ~eww~, however
many of those are now built into Emacs. So I have trimmed down my
file to provide only these commands:

- ~minimal-eww-visit-url-on-page~ :: Use minibuffer completion to select
...
- ~minimal-eww-find-feed~ :: Produce a bespoke buffer listing the
  RSS/Atom feeds provided by the current website.

- ~minimal-eww-open-in-other-window~ :: Like ~eww~, but operate in a new
...
*** The =minimal-emacs-web.el= RSS/Atom reader (~elfeed~)
:PROPERTIES:
...
*** The =minimal-emacs-web.el= settings for the IRC client (~rcirc~)
:PROPERTIES:
...
function ~minimal-common-auth-get-field~ to extract a password from my
=.authinfo.gpg= file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =minimal-emacs-email.el= basic settings (including =authinfo=)]]).
...
** The =minimal-emacs-which-key.el= module
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-which-key.el" :mkdirp yes
(minimal-emacs-configure
...
** The =minimal-emacs-icons.el= module (~nerd-icons~ for various packages)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-emacs-modules/minimal-emacs-icons.el" :mkdirp yes
;;; Icons
(when minimal-display-graphic-p
  (minimal-emacs-configure
...
** The =minimal-abbrev.el= library
:PROPERTIES:
:CUSTOM_ID: h:ddb9c32c-2114-4347-9f76-5160e590b5dc
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-abbrev.el" :mkdirp yes
...
** The =minimal-comment.el= library
:PROPERTIES:
:CUSTOM_ID: h:7affcbca-0808-4159-8eab-faa7973aefc7
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-comment.el" :mkdirp yes
...
** The =minimal-common.el= library
:PROPERTIES:
:CUSTOM_ID: h:3fccfadf-22e9-457f-b9fd-ed1b48600d23
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-common.el" :mkdirp yes
...
** The =minimal-dired.el= library
:PROPERTIES:
:CUSTOM_ID: h:92cfefcf-8c20-43cd-b47c-ffe0c5fe1887
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-dired.el" :mkdirp yes
...
** The =minimal-ediff.el= library
:PROPERTIES:
:CUSTOM_ID: h:3937b784-af0f-47c0-8c89-c74d90b00b8c
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-ediff.el" :mkdirp yes
...
** The =minimal-elisp.el= library
:PROPERTIES:
:CUSTOM_ID: h:9d63bbb2-1ee4-44aa-8601-8728f9f857ec
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-elisp.el" :mkdirp yes
...
** The =minimal-eww.el= library
:PROPERTIES:
:CUSTOM_ID: h:abc20037-7a4f-4555-809a-dc4165c5db6a
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-eww.el" :mkdirp yes
...
** The =minimal-minibuffer= library
:PROPERTIES:
:CUSTOM_ID: h:761861c9-9a0e-4c97-b98d-20f13a6b3b79
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-minibuffer.el" :mkdirp yes
...
** The =minimal-modeline.el= library
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-modeline.el" :mkdirp yes
...
** The =minimal-notmuch.el= library
:PROPERTIES:
:CUSTOM_ID: h:92ee86b8-0ead-4080-963f-3fbc9f931e3e
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-notmuch.el" :mkdirp yes
...
** COMMENT The =minimal-orderless.el= library
:PROPERTIES:
:CUSTOM_ID: h:114c6e8c-697a-43e9-8fd9-52872997e1c9
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-orderless.el" :mkdirp yes
;;; minimal-orderless.el --- Extensions for Orderless -*- lexical-binding: t -*-
...
(defun minimal-orderless-literal (word _index _total)
...
** The =minimal-pair.el= library
:PROPERTIES:
:CUSTOM_ID: h:0a3f076d-386d-47e6-8af9-f17d02d507cf
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-pair.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-prefix.el" :mkdirp yes
...
** The =minimal-project.el= library
:PROPERTIES:
:CUSTOM_ID: h:654cfe49-f59d-4a0d-84ee-094ddce06848
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-project.el" :mkdirp yes
...
** The =minimal-register.el= library
:PROPERTIES:
:CUSTOM_ID: h:5a429ac4-5b77-48ec-8903-20da9cf64a82
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-register.el" :mkdirp yes
...
** The =minimal-scratch.el= library
:PROPERTIES:
:CUSTOM_ID: h:efebaec4-7273-4c5a-a4c4-9f99df0e46bb
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-scratch.el" :mkdirp yes
...
** The =minimal-search.el= library
:PROPERTIES:
:CUSTOM_ID: h:0cab525d-fcbc-478c-b914-bbd69e33c635
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-search.el" :mkdirp yes
...
** The =minimal-shell.el= library
:PROPERTIES:
:CUSTOM_ID: h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-shell.el" :mkdirp yes
...
** The =minimal-simple.el= library
:PROPERTIES:
:CUSTOM_ID: h:5f78e837-0d27-4390-bd9a-6d0bca57fa50
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-simple.el" :mkdirp yes
...
** The =minimal-spell.el= library
:PROPERTIES:
:CUSTOM_ID: h:551718b7-fb2d-4a4a-804e-3118001b96ec
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-spell.el" :mkdirp yes
...
** The =minimal-vertico.el= library
:PROPERTIES:
:CUSTOM_ID: h:3796a4c9-8659-4782-8aaa-3cf4e950927d
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-vertico.el" :mkdirp yes
...
** The =minimal-window.el= library
:PROPERTIES:
:CUSTOM_ID: h:35b8a0a5-c447-4301-a404-bc274596238d
:END:

#+begin_src emacs-lisp :tangle "minimal-lisp/minimal-window.el" :mkdirp yes
...
because I can at any moment stop editing this =README.org= file
...
the ~outshine~ package ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =minimal-emacs-langs.el= settings for ~outline-minor-mode~]]).
...
Furthermore, I can use the ~consult-outline~ or ~minimal-search-outline~
command to navigate to a heading with minibuffer completion.

What the =README.org= provides is an /optional single point of
...
The =README.org= *does not introduce a dependency on Org mode*.
This file is not loaded at startup. I can remove =README.org= at
