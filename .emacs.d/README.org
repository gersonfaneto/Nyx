...
commands, one to store a register, and another to load it.
...
([[#h:0d49218d-4199-4847-9765-60a3f94e515d][The =minimal-emacs-completion.el= settings for completion category overrides]]).
...
- The ~find-file~ navigation makes it harder to select a directory.
  ...
  is possible thanks to the ~partial-completion~ style ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =minimal-emacs-completion.el= settings for completion styles]])).
...
*** The =minimal-emacs-completion.el= for in-buffer completion popup (~corfu~)
:PROPERTIES:
...
*** COMMENT The =minimal-emacs-completion.el= for in-buffer completion hints (~completion-preview-mode~)
:PROPERTIES:
...
tdiagnostics in the form of a standalone buffer as well as inline
