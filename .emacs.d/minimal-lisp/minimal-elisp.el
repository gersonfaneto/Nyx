;;; minimal-elisp.el --- Emacs Lisp extras for my dotemacs -*- lexical-binding: t -*-
...
;;;###autoload
(defun minimal-elisp-eval-and-print-last-sexp ()
...
(defvar-keymap minimal-elisp-macroexpand-mode-map
  :doc "Key map for `minimal-elisp-macroexpand-mode'."
  :parent special-mode-map)

(define-derived-mode minimal-elisp-macroexpand-mode emacs-lisp-mode "MacroExpand"
...
;;;###autoload
(defun minimal-elisp-pp-macroexpand-last-sexp ()
...
Now use `display-buffer-alist' like the Lisp gods intended."
  ...
  (if-let* ((thing (thing-at-point 'sexp :no-properties))
            ...
            (buffer (get-buffer-create "*minimal-elisp-macroexpand*"))
            (inhibit-read-only t))
      (progn
        (with-current-buffer buffer
          (erase-buffer)
          (insert (format "%S" (macroexpand-1 expression)))
          (minimal-elisp-macroexpand-mode)
          (pp-buffer))
        (display-buffer buffer))
    (user-error "No expression to macroexpand")))

(provide 'minimal-elisp)
;;; minimal-elisp.el ends here
