;;; minimal-register.el --- Extensions for project.el -*- lexical-binding: t -*-
...
;;;###autoload
(defun minimal-simple-file-to-register (register)
...
(defvar minimal-simple-file-to-register-jump-hook nil
  "Normal hook called after jumping to a file register.
See `minimal-simple-file-to-register'.")
...
outright."
  (cond
   ((eq (aref val 0) 'file-with-point)
    ...
    (run-hooks 'minimal-simple-file-to-register-jump-hook))
   (t (cl-call-next-method val delete))))

;;;; Do-What-I-Mean commands

;;;###autoload
(defun minimal-register-add-dwim (register &optional number)
...
Otherwise, use `minimal-simple-file-to-register'.

Also see `minimal-register-use-dwim'."
  (interactive
   (list
  ...
  (cond
   ...
   (t
    (minimal-simple-file-to-register register)
    (message "Copied current point in file to register `%c'" register))))

;;;###autoload
(defun minimal-register-use-dwim (register)
...
;;;; The `minimal-register-persist-mode'

(defcustom minimal-register-save-file (locate-user-emacs-file "minimal-register.eld")
  "File to store registers to and retrieve from."
  :type 'file)

(defun minimal-register-store ()
  "Write `register-alist' to `minimal-register-save-file'."
  (with-temp-file minimal-register-save-file
    (insert ";; Auto-generated file: DO NOT EDIT -*- mode: emacs-lisp -*-\n")
    (pp register-alist (current-buffer))))

(defun minimal-register-load ()
  "Read `minimal-register-save-file' and return its contents."
  (with-temp-buffer
    (when (file-exists-p minimal-register-save-file)
      (insert-file-contents minimal-register-save-file)
      (read (buffer-string)))))

(defun minimal-register-watcher (symbol newval operation where)
  (when (and (eq operation 'set) (null where) newval)
    (minimal-register-store)))

;;;###autoload
(define-minor-mode minimal-register-persist-mode
  "When enabled save `register-alist' to `minimal-register-save-file' when a change occurs."
  :globat t
  (if minimal-register-persist-mode
      (progn
        (add-variable-watcher 'register-alist #'minimal-register-watcher)
        (add-hook 'kill-emacs-hook #'minimal-register-store))
    (remove-variable-watcher 'register-alist #'minimal-register-watcher)
    (remove-hook 'kill-emacs-hook #'minimal-register-store)))

(provide 'minimal-register)
;;; minimal-register.el ends here
