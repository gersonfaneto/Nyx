;;; minimal-org.el --- Tweaks for my org-mode configurations -*- lexical-binding: t -*-
...
(defun minimal-org--capture-no-delete-windows (&rest args)
  "Apply ARGS while ignoring `delete-other-windows'."
  (cl-letf (((symbol-function 'delete-other-windows) 'ignore))
    (apply args)))

;; Same source as above
(advice-add 'org-capture-place-template :around 'minimal-org--capture-no-delete-windows)
(advice-add 'org-add-log-note :around 'minimal-org--capture-no-delete-windows)

;;;;; Custom function to select a project to add to

(defun minimal-org--get-outline (&optional file)
...
(defvar minimal-org-outline-history nil
  "Minibuffer history for `minimal-org-outline-prompt'.")

(defun minimal-org-outline-prompt (&optional file)
  "Prompt for outline among headings retrieved by `minimal-org--get-outline'.
With optional FILE use the outline of it, otherwise use that of
the current file."
  (let ((current-file (or file buffer-file-name))
        (default (car minimal-org-outline-history)))
    (completing-read
     (format-prompt
      (format "Select heading inside `%s': "
              (propertize (file-name-nondirectory current-file) 'face 'error))
      default)
     (minimal-common-completion-table-no-sort 'imenu (minimal-org--get-outline current-file))
     nil :require-match nil 'minimal-org-outline-history default)))

(defvar minimal-org-file-history nil
  "Minibuffer history of `minimal-org-file-prompt'.")

(defun minimal-org--not-useful-p (file)
...
(defun minimal-org-file-prompt ()
  "Select a file in the `org-directory'."
  (if-let* ((dir org-directory)
            (files (directory-files-recursively org-directory ".*" nil))
            (files (seq-remove #'minimal-org--not-useful-p files)))
      (let ((default (car minimal-org-file-history)))
        (completing-read
         (format-prompt "Select file" default)
         (minimal-common-completion-table 'file files)
         nil :require-match nil 'minimal-org-file-history default))
    (user-error "There are no files in the `org-directory'")))

;;;###autoload
(defun minimal-org-select-heading-in-file ()
...
there."
  ...
  (interactive)
  (pcase-let* ((file (minimal-org-file-prompt))
               (line-with-heading (minimal-org-outline-prompt file))
...
(defalias 'minimal-org-goto-heading-in-file 'minimal-org-select-heading-in-file
 "Alias for `minimal-org-select-heading-in-file'.")

;;;; Org clock report

(defvar minimal-org-clock--template-with-effort
...
(defvar minimal-org-clock--template-no-effort
...
(defvar minimal-org-clock--ranges
...
(defvar minimal-org-clock--report-range-history nil
  "Minibuffer history for `minimal-org-clock--report-range-prompt'.")

(defun minimal-org-clock--report-range-prompt ()
  "Prompt for a clock table range among `minimal-org-clock--ranges'."
  (let ((default (car minimal-org-clock--report-range-history)))
    (completing-read
     (format-prompt "Select a time range for the clock" default)
     minimal-org-clock--ranges nil :require-match nil 'minimal-org-clock--report-range-history
     default)))

(defun minimal-org-clock--get-report (scope)
...
use the entire file's contents.  Else use those of the current subtree."
  (let ((buffer (get-buffer-create "*minimal-org-custom-clock-report*")))
    (save-restriction
      (unwind-protect
          (progn
            (pcase scope
            ...
            (let ((contents (buffer-substring (point-min) (point-max))))
              (with-current-buffer buffer
                ...
                (org-mode)
                (save-excursion
                  (insert (format "%s\n\n" minimal-org-clock--template-with-effort))
                  (insert contents))
                (save-excursion
                  (let ((range (minimal-org-clock--report-range-prompt)))
...
;;;###autoload
(defun minimal-org-clock-report-current-subtree-or-file (&optional whole-buffer)
...
When called interactively WHOLE-BUFFER is a prefix argument."
  (interactive "P")
  (when-let* ((buffer (minimal-org-clock--get-report (if whole-buffer 'file 'subtree))))
    (pop-to-buffer buffer)))

;;;###autoload
(defun minimal-org-clock-select-heading-and-clock-report ()
  "Select a heading in a file and do a clock report for it in a new buffer."
  (interactive)
  (call-interactively 'minimal-org-select-heading-in-file)
  (call-interactively 'minimal-org-clock-report-current-subtree-or-file))

;;;;; Coaching-related Org custom clocking

;; TODO 2024-12-15: This sort of thing must exist in Org, but I did
;; not find it.
(defun minimal-org--timestamp-to-time (string)
  "Return time object of STRING timestamp."
  (org-timestamp-to-time (org-timestamp-from-string string)))

(defun minimal-org-coach--get-entries (todo-keyword string since)
            ...
            (org-map-entries
             (lambda ()
               (when-let* ((case-fold-search t)
                           (started (minimal-org--timestamp-to-time (or (org-entry-get nil "DEADLINE") (org-entry-get nil "SCHEDULED"))))
                           (closed (minimal-org--timestamp-to-time (org-entry-get nil "CLOSED")))
...
(defvar minimal-org-coach--name-history nil
  "Minibuffer history of `minimal-org-coach--name-prompt'.")

(defun minimal-org-coach--name-prompt ()
  "Prompt for name of person."
  (let ((default (car minimal-org-coach--name-history)))
    (read-string
     (format-prompt "Name of person" default)
     nil 'minimal-org-coach--name-history default)))

;;;###autoload
(defun minimal-org-coach-report (name since)
  "Produce clock report for coaching with person of NAME.
SINCE is the date (of time 00:00) to count from until now."
  (interactive
   (list
    (minimal-org-coach--name-prompt)
    (format "[%s]" (org-read-date))))
  (if-let* ((since-object (minimal-org--timestamp-to-time since))
            (entries (minimal-org-coach--get-entries "done" name since-object))
            (buffer (get-buffer-create "*minimal-org-coach-entries*")))
      (with-current-buffer (pop-to-buffer buffer)
        ...
        (goto-char (point-min))
        (save-excursion
          (insert (format "%s\n\n" minimal-org-clock--template-no-effort)))
        (save-excursion
          (goto-char (line-end-position))
...
;;;###autoload
(defun minimal-org-agenda-format-date-aligned (date)
...
(defun minimal-org-agenda-include-priority-no-timestamp ()
...
(defvar minimal-org-custom-daily-agenda
  ...
  `((tags-todo "*"
               ((org-agenda-overriding-header "Important tasks without a date\n")
                ...
                ;; `minimal-org-agenda-include-priority-no-timestamp'.
                ;;
                ;; (org-agenda-skip-function '(org-agenda-skip-subtree-if nil '(timestamp)))
                ;; (org-agenda-skip-function
                ;;  `(org-agenda-skip-entry-if
                ;;    'notregexp ,(format "\\[#%s\\]" (char-to-string org-priority-highest))))
                (org-agenda-skip-function #'minimal-org-agenda-include-priority-no-timestamp)
...
(defun minimal-org-custom-agenda-minimal-asks-get-date ()
  "Return the timestamp of the current heading or nil.
For use in `minimal-org-custom-minimal-asks-agenda'."
  (when-let* ((timestamp (or (org-entry-get nil "DEADLINE") (org-entry-get nil "SCHEDULED")))
              (time (minimal-org--timestamp-to-time timestamp)))
    (format-time-string "%10A	%e %10B %6R" time)))

(defvar minimal-org-custom-minimal-asks-agenda
  '((tags-todo "protasks"
               ((org-agenda-overriding-header "Prot Asks\n")
                (org-agenda-prefix-format '((tags . "%(minimal-org-custom-agenda-minimal-asks-get-date)	")))
                (org-agenda-sorting-strategy '(deadline-up))
                (org-agenda-remove-tags t)
                (org-agenda-block-separator nil))))
  "Custom agenda for use in `org-agenda-custom-commands'.")

(defun minimal-org-agenda-set-outline ()
  "Set `outline-regexp' for my Org agenda buffers."
  (when (derived-mode-p 'org-agenda-mode)
    (setq-local outline-regexp "^\\([\s\t]+\\)\\([A-Z]+ \\|[0-9]+:[0-9]+ \\)\\(Deadline\\|Scheduled\\)")))

(add-hook 'org-agenda-mode-hook #'minimal-org-agenda-set-outline)

;;;;; agenda appointments

(defvar minimal-org-agenda-after-edit-hook nil
  "Hook that runs after select Org commands.
To be used with `advice-add'.")

(defun minimal-org--agenda-after-edit (&rest _)
  "Run `minimal-org-agenda-after-edit-hook'."
  (run-hooks 'minimal-org-agenda-after-edit-hook))

(defvar minimal-org-after-deadline-or-schedule-hook nil
  "Hook that runs after `org--deadline-or-schedule'.
To be used with `advice-add'.")

(defvar minimal-org--appt-agenda-commands
  '( org-agenda-archive org-agenda-deadline org-agenda-schedule
     org-agenda-todo org-archive-subtree)
  "List of commands that run `minimal-org-agenda-after-edit-hook'.")

(dolist (fn minimal-org--appt-agenda-commands)
  (advice-add fn :after #'minimal-org--agenda-after-edit))

(defun minimal-org--after-deadline-or-schedule (&rest _)
  "Run `minimal-org-after-deadline-or-schedule-hook'."
  (run-hooks 'minimal-org-after-deadline-or-schedule-hook))

(defun minimal-org-org-agenda-to-appt ()
...
(dolist (hook '(org-capture-after-finalize-hook
                ...
                minimal-org-agenda-after-edit-hook))
  (add-hook hook #'minimal-org-org-agenda-to-appt))

(declare-function org--deadline-or-schedule "org" (arg type time))

(advice-add #'org--deadline-or-schedule :after #'minimal-org--after-deadline-or-schedule)

(add-hook 'minimal-org-after-deadline-or-schedule-hook #'minimal-org-org-agenda-to-appt)
...
;;;###autoload
(defun minimal-org-ox-html ()
  "Streamline HTML export."
  (interactive)
  (org-html-export-as-html nil nil nil t nil))

;;;###autoload
(defun minimal-org-ox-texinfo ()
...
(defun minimal-org--heading-to-id ()
...
(defun minimal-org--id-get-readable (&optional force)
  "Like `minimal-org--id-get' but use the heading wording to create and ID.
With optional FORCE, update the value even if one exists."
  (let* ((pos (point))
         (id (unless force (org-entry-get pos "CUSTOM_ID"))))
    (or (and id (stringp id) (string-match-p "\\S-" id))
        (and (setq id (minimal-org--heading-to-id))
...
;;;###autoload
(defun minimal-org-id-headlines ()
  "Add missing CUSTOM_ID to all headlines in current file."
  (interactive)
  (org-map-entries (lambda () (minimal-org--id-get))))

;;;###autoload
(defun minimal-org-id-headlines-readable (&optional force)
  "Like `minimal-org-id-headlines' but with readable IDs.
A readable identifier is one derived from the text of the heading.  In
theory, this may not be unique.

With optional FORCE, update the value even if one exists."
  (interactive "P")
  (org-map-entries (lambda () (minimal-org--id-get-readable force))))

;;;###autoload
(defun minimal-org-id-headline (&optional readable)
  "Add missing CUSTOM_ID to headline at point.
With optional prefix argument READABLE get a readable identifier derived
from the heading text instead of a UUID."
  (interactive "P")
  (funcall (if readable 'minimal-org--id-get-readable 'minimal-org--id-get)))

;;;###autoload
(defun minimal-org-get-dotemacs-link ()
  "Get URL to current heading in my dotemacs file."
  (declare (interactive-only t))
  (interactive)
  (if (and (derived-mode-p 'org-mode)
           (string-match-p "minimal-emacs\\.org\\'" buffer-file-name))
