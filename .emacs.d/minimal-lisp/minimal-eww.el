;;; minimal-eww.el --- Extensions for EWW -*- lexical-binding: t -*-
...
(defgroup minimal-eww nil
  "Tweaks for EWW."
  :group 'eww)

(defun minimal-eww--get-urls ()
...
(defun minimal-eww-buffer-url-prompt ()
  "Prompt for a url in the current buffer."
  (when-let* ((link-data (minimal-eww--get-urls))
              (candidates (mapcar
                           (pcase-lambda (`(,position ,name ,_))
                             (format "%s	%s" position name))
                           link-data))
              (table (minimal-common-completion-table-no-sort nil candidates))
              (selection
               (completing-read
...
(defun minimal-eww-visit-url-on-page (&optional new-buffer)
...
buffer instead of the current one."
  ...
  (if-let* ((data (minimal-eww-buffer-url-prompt))
            (url (nth 2 data)))
      (eww url new-buffer)
    (error "Cannot find URL in data `%s'" data)))

(defun minimal-eww-jump-to-url-on-page ()
...
(defvar minimal-eww--occur-feed-regexp
  (concat "\\(rss\\|atom\\)\\+xml.\\(.\\|\n\\)"
          ".*href=[\"']\\(.*?\\)[\"']")
  "Regular expression to match web feeds in HTML source.")

(defun minimal-eww-find-feed ()
  ...
  (let* ((url (or (plist-get eww-data :start)
    ...
    (with-temp-buffer
      (insert source)
      (occur-1 minimal-eww--occur-feed-regexp "\\3" (list (current-buffer)) buf-name))
      ...
      (with-current-buffer (get-buffer buf-name)
        (let ((inhibit-read-only t)
          ...
          (while (< (point) (point-max))
            ...
            (when (and (looking-at minimal-common-url-regexp)
                       (not (looking-at (format "%s.*" url))))
              (insert base-url))
            (forward-line 1)))))))

;;;###autoload
(defun minimal-eww-open-in-other-window ()
