;;; minimal-ediff.el --- Ediff extensions for my dotemacs -*- lexical-binding: t -*-
...
(defun minimal-ediff-store-layout ()
  "Store current frame window configuration as a frame parameter.
Add this function to the `ediff-before-setup-hook'.

Also see `minimal-ediff-restore-layout'."
  (let ((frame (selected-frame)))
    (set-frame-parameter
     frame
     'minimal-ediff-last-layout
     (current-window-configuration frame))))

(defun minimal-ediff-restore-layout ()
  "Restore the frame's window configuration.
Add this function to the `ediff-quit-hook'.

Also see `minimal-ediff-store-layout'."
  (if-let* ((layout (frame-parameter (selected-frame) 'minimal-ediff-last-layout)))
...
(defun minimal-ediff-visible-buffers-2 ()
...
(defun minimal-ediff-visible-buffers-3 ()
...
(provide 'minimal-ediff)
;;; minimal-ediff.el ends here
