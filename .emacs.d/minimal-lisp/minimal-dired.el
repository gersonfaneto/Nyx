;;; minimal-dired.el --- Extensions to dired.el for my dotemacs -*- lexical-binding: t -*-
...
(defgroup minimal-dired ()
  "Extensions for Dired."
  :group 'dired)

;;;; Flat Dired listing

(defvar minimal-dired-regexp-history nil
  "Minibuffer history of `minimal-dired-regexp-prompt'.")

(defun minimal-dired-regexp-prompt ()
  (let ((default (car minimal-dired-regexp-history)))
    (read-regexp
     (format-prompt "Files matching REGEXP" default)
     default 'minimal-dired-regexp-history)))

(defun minimal-dired--get-files (regexp)
  "Return files matching REGEXP, recursively from `default-directory'."
  (directory-files-recursively default-directory regexp nil))

;;;###autoload
(defun minimal-dired-search-flat-list (regexp)
  "Return a Dired buffer for files matching REGEXP.
Perform the search recursively from the current directory."
  (interactive (list (minimal-dired-regexp-prompt)))
  (if-let* ((files (minimal-dired--get-files regexp))
            (relative-paths (mapcar #'file-relative-name files)))
      (dired (cons (format "minimal-flat-dired for `%s'" regexp) relative-paths))
...
(defcustom minimal-dired-always-external-regexp
  "\\(mkv\\|mp4\\|mp4\\|ogg\\|m4a\\|webm\\)"
  "Regular expression of file extensions to open externally.
The test is performed by `minimal-dired-open-dwim', which then
...
;; note.
(defun minimal-dired-open-dwim (files)
  "Open FILES using the appropriate program."
  (interactive (list (dired-get-marked-files)))
  (if-let* ((extension (file-name-extension (car files)))
            ((string-match-p extension minimal-dired-always-external-regexp))
            (guess (dired-guess-default files))
            (program (if (listp guess) (car guess) guess)))
      (dired-do-async-shell-command program nil files)
    (find-file (car files))))

(defvar minimal-dired--limit-hist '()
  "Minibuffer history for `minimal-dired-limit-regexp'.")

;;;###autoload
(defun minimal-dired-limit-regexp (regexp omit)
   ...
   (list
    (read-regexp
     (concat "Files "
             ...
             "matching PATTERN: ")
     nil 'minimal-dired--limit-hist)
    current-prefix-arg))
  (dired-mark-files-regexp regexp)
  (unless omit (dired-toggle-marks))
  (dired-do-kill-lines)
  (add-to-history 'minimal-dired--limit-hist regexp))

(defvar minimal-dired-grep-marked-files-history nil
  "Minibuffer history for `minimal-dired-grep-marked-files-prompt'.")

(defun minimal-dired-grep-marked-files-prompt ()
  "Prompt for string to search for with `minimal-dired-grep-marked-files'."
  (read-string
   "grep for PATTERN in marked files: "
   nil 'minimal-dired-grep-marked-files-history))

;; Also see `minimal-search-grep' from minimal-search.el.
;;;###autoload
(defun minimal-dired-grep-marked-files (files regexp)
  ...
  (interactive
   (if-let* ((marks (dired-get-marked-files 'no-dir))
       ...
       (list
        marks
        (minimal-dired-grep-marked-files-prompt))
     (user-error "Mark multiple files"))
   dired-mode)
  (let ((buffer-name (format "*minimal-dired-grep-marked for `%s'*" regexp)))
    (compilation-start
     (concat
...
;;;###autoload
(defun minimal-dired-subdirectory-previous (&optional arg)
...
;;;###autoload
(defun minimal-dired-remove-inserted-subdirs ()
  "Remove all inserted Dired subdirectories."
  (interactive)
  (goto-char (point-max))
  (while (and (minimal-dired-subdirectory-previous)
...
(defun minimal-dired--dir-list (list)
...
(defun minimal-dired--insert-dir (dir &optional flags)
...
;;;###autoload
(defun minimal-dired-insert-subdir (&optional arg)
...
inserted subdirectories."
  ...
  (let* ((name (dired-get-marked-files))
    ...
    (cond  ; NOTE 2021-07-20: `length>', `length=' are from Emacs28
     ((eq arg 16)
      (minimal-dired-remove-inserted-subdirs))
     ((and (length> name 1) (minimal-dired--dir-list name))
      (mapc (lambda (file)
              (when (file-directory-p file)
                (minimal-dired--insert-dir file flags)))
            name))
     ((and (length= name 1) (file-directory-p (car name)))
      (minimal-dired--insert-dir (car name) flags))
     (t
      (let ((selection (read-directory-name "Insert directory: ")))
        (minimal-dired--insert-dir selection flags))))))

(defun minimal-dired--imenu-prev-index-position ()
  "Find the previous file in the buffer."
  (let ((subdir minimal-dired--directory-header-regexp))
    (re-search-backward subdir nil t)))

(defun minimal-dired--imenu-extract-index-name ()
...
;;;###autoload
(defun minimal-dired-setup-imenu ()
  "Configure imenu for the current Dired buffer.
Add this to `dired-mode-hook'."
  (set (make-local-variable 'imenu-prev-index-position-function)
       'minimal-dired--imenu-prev-index-position)
  (set (make-local-variable 'imenu-extract-index-name-function)
       'minimal-dired--imenu-extract-index-name))

(provide 'minimal-dired)
;;; minimal-dired.el ends here
