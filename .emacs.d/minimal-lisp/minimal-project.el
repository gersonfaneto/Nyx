;;; minimal-project.el --- Extensions for project.el -*- lexical-binding: t -*-
...
(defun minimal-project--switch (directory &optional command)
...
(defun minimal-project--frame-names ()
  "Return a list of frame names."
  (mapcar #'car (make-frame-names-alist)))

(defvar minimal-project-switch-hook nil
  "Normal hook called after `minimal-project-switch'.")

;;;###autoload
(defun minimal-project-switch (directory)
...
the project in DIRECTORY using `project-dired'."
  ...
  (let ((name (file-name-nondirectory (directory-file-name directory))))
    (if (member name (minimal-project--frame-names))
        (select-frame-by-name name)
      (minimal-project--switch directory 'project-dired))
    (run-hooks 'minimal-project-switch-hook)
    (setq this-command 'project-switch-project)))

;;;; Produce a VC root log for the project

(defun minimal-project-rename-vc-root-log (&rest _)
...
(advice-add #'vc-print-root-log :after #'minimal-project-rename-vc-root-log)
...
(defun minimal-project-in-tab--get-tab-names (&optional frame)
...
(defun minimal-project-in-tab--create-tab (directory name)
...
;;;###autoload
(defun minimal-project-in-tab (directory)
...
Use this as an alternative to `project-switch-project'."
  ...
  (project--remember-dir directory)
  (let ((name (file-name-nondirectory (directory-file-name directory))))
    (if (member name (minimal-project-in-tab--get-tab-names))
        (tab-switch name)
      (minimal-project-in-tab--create-tab directory name))
    (setq this-command 'project-switch-project)))

;;;###autoload
(defun minimal-project-maybe-in-tab ()
  "Switch to project depending on `beframe-mode'.
When the mode is enabled (the expected behaviour), use the command
`minimal-project-switch'.  Otherwise, do `minimal-project-in-tab'."
  (interactive)
  (call-interactively
   (if (bound-and-true-p beframe-mode)
       'minimal-project-switch
     'minimal-project-in-tab)))

;;;; Set up a project root

;; I don't actually have a use-case for `minimal-project-find-root',
;; but I wrote it once so I keep it here in case I ever need it.
;; Use it like this: (minimal-project-find-root c-mode "Makefile")
(defmacro minimal-project-find-root (mode file)
...
is a string."
  (let ((project-find-fn (intern (format "project-find-%s-root" mode)))
        (major-mode-fn (intern (format "minimal-%s-project-find-function" mode)))
    ...
    `(progn
       ...
       (defun ,(intern (format "minimal-%s-project-find-function" mode)) ()
...
(defun minimal-project-compilation-read-command (command)
...
(advice-add #'compilation-read-command :override #'minimal-project-compilation-read-command)

(provide 'minimal-project)
;;; minimal-project.el ends here
