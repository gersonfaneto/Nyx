;;; minimal-vertico.el --- Custom Vertico extras -*- lexical-binding: t -*-
...
(defvar minimal-vertico-multiform-minimal
  '(unobtrusive
...
command or use the commands `minimal-vertico-private-next' and
`minimal-vertico-private-previous', which toggle the vertical view
automatically.")

(defvar minimal-vertico-multiform-maximal
  '((vertico-count . 10)
    (vertico-preselect . directory)
    (vertico-resize . t))
  "List of configurations for maximal Vertico multiform.")

(defun minimal-vertico-private-next ()
  "Like `vertico-next' but toggle vertical view if needed.
This is done to accommodate `minimal-vertico-multiform-minimal'."
  (interactive)
...
(defun minimal-vertico-private-previous ()
...
(defun minimal-vertico-private-complete ()
...
This is done to accommodate `minimal-vertico-multiform-minimal'."
  ...
  (if (and vertico-unobtrusive-mode (> vertico--total 1))
      (progn
        (minibuffer-complete)
        (minimal-vertico-private-next))
    (vertico-insert)))

(defun minimal-vertico-private-exit ()
  "Exit with the candidate if `minimal-vertico-multiform-minimal'.
...
(provide 'minimal-vertico)
;;; minimal-vertico.el ends here
