...
(use-package mu4e
  ...
  (setq mu4e-contexts
        `(,@(mapcar
             (lambda (context)
               (let ((address (minimal-common-auth-get-field context :user)))
                 (make-mu4e-context
                  :name context
  ...
  (minimal-emacs-keybind mu4e-headers-mode-map
  ...
  (minimal-emacs-keybind mu4e-view-mode-map
