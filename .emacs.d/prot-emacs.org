...
- [[#h:bd3f7a1d-a53d-4d3e-860e-25c5b35d8e7e][The =prot-emacs-completion.el= section to configure completion annotations (~marginalia~)]]
...
** The =init.el= macro to add autoloads (~prot-emacs-autoload~)
:PROPERTIES:
:CUSTOM_ID: h:915cab3f-8cdc-46ab-a608-290992be2cfd
:END:

The ~prot-emacs-autoload~ will generate autoloads for one or more
...
#+begin_example emacs-lisp
(prot-emacs-autoload test-fn-1 "test-file")
;; => (autoload #'test-fn-1 "test-file")

(prot-emacs-autoload (test-fn-1 test-fn-2) "test-file")
;; => (progn
;; =>   (autoload #'test-fn-1 "test-file")
...
:PROPERTIES:
:CUSTOM_ID: h:950127eb-6bac-4587-baa0-14ea73f3bee1
:END:
**** The =prot-emacs-doom-themes.el= module
:PROPERTIES:
:CUSTOM_ID: h:2b32a919-7eb3-4e7e-81f5-d5038ad8d4fe
:END:

#+begin_src emacs-lisp :tangle "prot-emacs-modules/prot-emacs-doom-themes.el"
;;; The Doom Themes
...
*** The =prot-emacs-essentials.el= settings for registers (~prot-register~)
:PROPERTIES:
...
My =prot-register.el= library defines a new register method to store
...
defines the ~prot-register-persist-mode~ which saves the registers to
a file (controlled by ~prot-register-save-file~) whenever I make a
change to them.
...
definitions, where applicable.  When Xref finds matches across files,
it groups them per file (and each file gets a decorative icon on the
side ([[#h:4efde779-3ef0-40b7-82e4-70f71b321428][The =prot-emacs-icons.el= module (~prot-icons~)]])).

The =grep.el= is a wrapper for the Unix program of the same name. Not
much to add there. While =re-builder.el= defines a command that lets
us write a regexp that matches against the current buffer, allowing us
to test it live.
...
Note that my Dired buffers not only hide details by default: they also
have decorative "icons" next to each file in the listing ([[#h:4efde779-3ef0-40b7-82e4-70f71b321428][The =prot-emacs-icons.el= module (~prot-icons~)]]).
...
*** COMMENT The =prot-emacs-dired.el= section about ~mandoura~ (~mpv~ media player)
:PROPERTIES:
...
(prot-emacs-configure
  ...
  (prot-emacs-keybind vc-git-log-view-mode-map
...
changes I make here are not necessary. One of them is the inclusion of
decorative icons next to file names ([[#h:4efde779-3ef0-40b7-82e4-70f71b321428][The =prot-emacs-icons.el= module (~prot-icons~)]]).
...
*** The =prot-emacs-org.el= indent and default folding settings for Org
:PROPERTIES:
...
- To help me find my tasks more easily, I rely on tags. Those are
  ...
  everything ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =prot-emacs-org.el= Org agenda settings]]).
...
Starting with Emacs 30, ~which-key~ is built into Emacs. This is a
minor mode that automatically displays a listing of all the keys that
complete the current incomplete key sequence. For example, if we type
=C-x= and wait for ~which-key-idle-delay~, we will get a popup window
with all the key bindings that start with =C-x=, such as =C-x C-f=.
The listing includes the names of the commands bound to each key. This
is useful as a learning tool, as we can then call that command by name
via =M-x= or even bind it to another key.
...
** The =prot-emacs-icons.el= module (~prot-icons~)
:PROPERTIES:
:CUSTOM_ID: h:4efde779-3ef0-40b7-82e4-70f71b321428
:END:

I used to have a setup for Nerd Fonts that would add iconography to
various interfaces. I never really like it, but understood the benefit
of it for screen sharing: the observer can better understand what is
on display.

The =prot-icons.el= defines "icons", which are just plain text
characters with some associated colours, and adds them to various
places ([[#h:99294d20-ffe5-4d0b-ac62-04fb39d1ba82][The =prot-icons.el= library]]). Though anything related to the
minibuffer and completions is done by =prot-minibuffer.el= and the
~completion-category-overrides~, while the relevant mode line parts
are handled by =prot-modeline.el=:

- [[#h:0d49218d-4199-4847-9765-60a3f94e515d][The =prot-emacs-completion.el= settings for completion category overrides]].
- [[#h:761861c9-9a0e-4c97-b98d-20f13a6b3b79][The =prot-minibuffer= library]].
...
packages for Emacs started out as custom libraries like those
([[#h:e289a614-4f17-4d6c-a028-42fe45aebe66][The init.el arrangements for my own modules and custom libraries]]).
...
not test it as much and do not try to make it work for all cases.
...
** The =prot-icons.el= library
:PROPERTIES:
:CUSTOM_ID: h:99294d20-ffe5-4d0b-ac62-04fb39d1ba82
:END:
