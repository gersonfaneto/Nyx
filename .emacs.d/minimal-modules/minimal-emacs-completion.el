...
(minimal-emacs-configure
  ...
  (minimal-minibuffer-missing-categories-mode 1)
  ...
  (let* ((eager-update-properties '((styles . (basic substring flex))
    ...
    (setq completion-category-overrides
          `((file . ((styles . (partial-completion))
                     (eager-display . nil)
                     (eager-update . t)
                     (group-function . ,#'minimal-minibuffer-file-group)
                     (display-sort-function . ,#'minimal-minibuffer-file-sort-directories-first)))
            (bookmark . ((eager-display . nil)
                         (eager-update . nil)
                         (annotation-function . ,#'minimal-minibuffer-bookmark-annotate)))
            (project-file . (,@eager-update-properties
                             (group-function . ,#'minimal-minibuffer-file-group)))
            (library . (,@eager-update-properties
                        (annotation-function . ,#'minimal-minibuffer-library-annotate)
                        (display-sort-function . ,#'minimal-minibuffer-library-sort)))
            (symbol-help . (,@eager-update-properties
                            (display-sort-function . ,#'minimal-minibuffer-symbol-sort)))
            (buffer . (,@eager-update-properties
                       (annotation-function . ,#'minimal-minibuffer-buffer-annotate)))
            (command . ((affixation-function . nil)
                        (annotation-function . ,#'minimal-minibuffer-command-annotate)))
...
;;; Orderless completion style (and minimal-orderless.el)
(when minimal-emacs-completion-extras
  (minimal-emacs-configure
    (minimal-emacs-install orderless)
    (require 'orderless)
    ...
    (minimal-emacs-keybind minibuffer-local-completion-map
      "SPC" nil
      "?" nil)))
...
(minimal-emacs-configure
  ...
  (unless minimal-emacs-completion-ui
    (minimal-minibuffer-completions-mode 1)

    (minimal-emacs-keybind completion-list-mode-map
      "h" #'minimal-minibuffer-completions-describe-at-point ; "Help" mnemonic
      "c" #'minimal-minibuffer-choose-completion-no-exit ; "Choose" mnemonic
      "TAB" #'minimal-minibuffer-choose-completion-dwim
      "RET" #'minimal-minibuffer-choose-completion-exit)))
...
(minimal-emacs-configure
  ...
  (minimal-emacs-abbrev global-abbrev-table
  ...
  (minimal-emacs-abbrev text-mode-abbrev-table
    "asciidoc"       "AsciiDoc"
    "auctex"         "AUCTeX"
    "cafe"           "café"
    "cliche"         "cliché"
  ...
  (with-eval-after-load 'org
    (minimal-emacs-abbrev org-mode-abbrev-table
      ";dev" "{{{development-version}}}"
      ";key" #'minimal-abbrev-org-macro-key
      ";cmd" #'minimal-abbrev-org-macro-key-command)
    (abbrev-table-put org-mode-abbrev-table :regexp "\\(?:^\\|[\t\s]+\\)\\(?1:[:;_].*\\|.*\\)"))

  (with-eval-after-load 'message
    (minimal-emacs-abbrev message-mode-abbrev-table
...
Development continues on GitHub with GitLab as a mirror."))
  ...
  ;; The `minimal-emacs-abbrev' macro, which simplifies how we use
  ...
  ;; templating system.
  (require 'minimal-abbrev)
  (minimal-emacs-abbrev global-abbrev-table
    "metime" #'minimal-abbrev-current-time
    "medate" #'minimal-abbrev-current-date
    "mejitsi" #'minimal-abbrev-jitsi-link
    ";time" #'minimal-abbrev-current-time
    ";date" #'minimal-abbrev-current-date
    ";jitsi" #'minimal-abbrev-jitsi-link)

  (minimal-emacs-abbrev text-mode-abbrev-table
    ";update" #'minimal-abbrev-update-html)
...
(when (and (eq minimal-emacs-completion-ui 'vertico)
           minimal-emacs-completion-extras
           minimal-display-graphic-p)
  (minimal-emacs-configure
    (minimal-emacs-install corfu)
...
(minimal-emacs-comment
  (minimal-emacs-configure
    ...
    (with-eval-after-load 'completion-preview
      (minimal-emacs-keybind completion-preview-active-mode-map
...
(when minimal-emacs-completion-extras
  (minimal-emacs-configure
    (minimal-emacs-install embark)

    (add-hook 'embark-collect-mode-hook #'minimal-common-truncate-lines-silently)

    (minimal-emacs-keybind minibuffer-local-map
      "C-c C-c" #'embark-collect
      "C-c C-e" #'embark-export)

    ;; Needed for correct exporting while using Embark with Consult commands.
    (minimal-emacs-install embark-consult)
...
(when minimal-emacs-completion-extras
  (minimal-emacs-configure
    (minimal-emacs-install marginalia)
    (setq marginalia-max-relative-age 0) ; absolute time
    (marginalia-mode 1)))

;;; The minibuffer user interface (mct, vertico, or none)
(when minimal-emacs-completion-ui
  (require
   (pcase minimal-emacs-completion-ui
     ('mct 'minimal-emacs-mct)
     ('vertico 'minimal-emacs-vertico))))

(provide 'minimal-emacs-completion)
