...
(minimal-emacs-configure
  ...
  (advice-add #'project-switch-project :after #'minimal-common-clear-minibuffer-message)

  (autoload #'minimal-project-maybe-in-tab "minimal-project")
  (define-key project-prefix-map (kbd "p") #'minimal-project-maybe-in-tab))
...
(minimal-emacs-configure
  ...
  (minimal-emacs-autoload
    (minimal-ediff-visible-buffers-2
     minimal-ediff-visible-buffers-3
     minimal-ediff-store-layout
     minimal-ediff-restore-layout)
    "minimal-ediff")
  ...
  ;; use-case.
  (minimal-emacs-keybind global-map
    "C-x v 2" #'minimal-ediff-visible-buffers-2
    "C-x v 3" #'minimal-ediff-visible-buffers-3)
  (add-hook 'ediff-before-setup-hook #'minimal-ediff-store-layout)
  (add-hook 'ediff-quit-hook #'minimal-ediff-restore-layout))
...
(minimal-emacs-configure
  ...
  (with-eval-after-load 'vc
    ...
    (minimal-emacs-keybind vc-dir-mode-map
      "t" #'vc-create-tag
      "O" #'vc-log-outgoing
...
(minimal-emacs-configure
  ...
  (with-eval-after-load 'magit
    (minimal-emacs-keybind magit-mode-map
      "C-w" #'nil
      "M-w" #'nil))
