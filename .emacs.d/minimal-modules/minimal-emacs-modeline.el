...
(minimal-emacs-configure
  (require 'minimal-modeline)
  (setq mode-line-compact nil) ; Emacs 28
  (setq mode-line-right-align-edge 'right-margin) ; Emacs 30
  (setq minimal-modeline-show-frame-name (alist-get 'undecorated initial-frame-alist))

  (setq-default mode-line-format
                '("%e"
                  minimal-modeline-kbd-macro
  ...
  (with-eval-after-load 'spacious-padding
    (defun prot/modeline-spacious-indicators ()
      "Set box attribute to `'minimal-modeline-indicator-button' if spacious-padding is enabled."
      (if (bound-and-true-p spacious-padding-mode)
          (set-face-attribute 'minimal-modeline-indicator-button nil :box t)
        (set-face-attribute 'minimal-modeline-indicator-button nil :box 'unspecified)))
...
(minimal-emacs-configure
  ...
  ;; NOTE 2025-10-26: I handle the indicator on my own via `minimal-modeline-which-function-indicator'.
  ...
  (with-eval-after-load 'minimal-modeline
          ...
          (ignore-errors
            (when-let* ((text (save-excursion
              ...
              (setq name (if (string-prefix-p ";" definition)
                             ""
                           (minimal-modeline-string-abbreviate-but-last definition 1))))))
         (t
          (when (null name)
          ...
          (minimal-modeline-string-cut-end name)))))
...
;;; Keycast mode
(minimal-emacs-configure
  (minimal-emacs-install keycast)
  (with-eval-after-load 'minimal-modeline
    (setq keycast-mode-line-format "%2s%k%c%R")
    (setq keycast-mode-line-insert-after 'minimal-modeline-vc-branch)
...
(provide 'minimal-emacs-modeline)
